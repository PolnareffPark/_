# -*- coding: utf-8 -*-
"""
test.py (leak-hard-guard, optuna-free, auto-leak-audit, anchor-reuse)
...
RUN_NAME = _os.environ.get("RUN_NAME", "r2_uplift_final_v5")

# 재현·탐색 기본 프리셋 (필요 시 RUN_ONLY_EXPR / RUN_WITH_EXPR로 오버라이드)
RUN_ONLY = [
    "S0",                  # 기준
    "S0+TE2",              # Δ TE만
    "S0+L1+PN1+DF1",       # 시퀀스/정규화/Δ
    "S0+M2",               # CUR_WARP 단조(+1)
    "S0+BAG",              # Bagging
    "S0+S1",               # Stratified split
    "S0+X2E_YJ",           # X2E: YJ
    "S0+X2E_RES",          # X2E: Residual 2-stage
    "S0+X2E_MOE",          # X2E: Pair expert MoE
]
RUN_WITH = [
    "S0+TE2+W2",           # Δ TE + 이분산
    "S0+TE2+W2+M2",        # + 단조
    "S0+L1+DF1+TE2+W2",    # 시퀀스/Δ/TE2/W2
    "S0+TE2+X2E_RES+M2",   # TE2 + X2E Residual + 단조
    "S0+TE2+X2E_YJ+M2",    # TE2 + X2E YJ + 단조_
]
...
def train_single_model(train_df, test_df, direction_key: str, flags: set, algo_name: str):
    ...
    if (direction_key == "X2E") and ("X2E_YJ" in flags):
        pt = fit_yj_transformer(tr["_target"].values)
        ytr_t = yj_transform(pt, tr["_target"].values)
        chosen_names, Xtr, Xte = select_features(Xtr_df, ytr_t, Xte_df, mandatory=mandatory, k=SELECTK_K)
        groups = tr[PLATE_COL].astype(str).values
        model, yhat_t = _fit_predict_model(Xtr, ytr_t, Xte, flags, algo_name, chosen_names,
                                           sample_weight=sample_weight, groups=groups)
        yhat = yj_inverse(pt, yhat_t)
        if "DELTA_TARGET" in flags: yhat = te[CUR_WARP].values + yhat
        return {"model": model, "features": chosen_names,
                "r2_tr": float(r2_score(tr["_target"].values, yj_inverse(pt, model.predict(Xtr)))),
                "r2_te": float(r2_score(te["_target"].values, yhat)),
                "rmse_tr": float(np.sqrt(mean_squared_error(tr["_target"].values, yj_inverse(pt, model.predict(Xtr))))),
                "rmse_te": float(np.sqrt(mean_squared_error(te["_target"].values, yhat))),
                "yte": te["_target"].values, "yhat": yhat}
