...
# ========================= [ USER CONFIG ] =========================
RUN_NAME = "r2_uplift_alias_v2"   # 동시 다실험 시 서로 다른 이름으로 변경
RUN_ONLY = []  # 예: ["S0","W2","TE2"]
RUN_WITH = ["S0", "S0+R1+W2", "S0+TE2+W2", "S0+S1+IW", "S0+R1+TE2+W2+BAG"]   # 예: ["S0+R1","R1+S1"]
...
def train_single_model(train_df, test_df, direction_key: str, flags: set, algo_name: str):
    ...
    # TE2: Δ-TE 특성 추가(누수 방지: train OOF, test는 full-train 매핑)
    if "PAIR_TE" in flags:
...
def prepare_train_test_leak_safe(df_base: pd.DataFrame, use_strat: bool, direction_key: str, split_anchor: str, flags: set):
    ...
    tr = add_sequence_lags(base_feature_engineering(tr_raw))
    te = add_sequence_lags(base_feature_engineering(te_raw))
...
def main():
    ...
    for key, flag_list in recipes.items():
        flags = set(flag_list)
        split_anchor = f"{RUN_NAME}__{'STRAT' if 'STRAT_SPLIT' in flags else 'BASE'}__anchor"
        ...
        # Strict roll-forward R² (교집합 Test 판)
        ...
        rf_strict = strict_rollforward_r2(final_df, set(e2x_res["test_plates"]), set(x2e_res["test_plates"]),
