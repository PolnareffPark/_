...
def compare_algorithms(df: pd.DataFrame, dataset_name: str):
    ...
    # 기본 숫자 피처만, EXCLUDE 제거 (여기서는 단순 비교용으로 전역 대치 없이 사용)
    X_all = df.drop(columns=[c for c in EXCLUDE_FROM_X if c in df.columns], errors="ignore")
    X = X_all.select_dtypes(include=[np.number]).copy().fillna(0.0)
    ...
    algos = {
        "XGBoost": XGBRegressor(
            ...
            reg_lambda=2.0, reg_alpha=0.5, min_child_weight=10
        ),
...
def train_model(df: pd.DataFrame,
    ...
    use_delta   = str(os.getenv("USE_DELTA_TARGET", "0")).lower() in ("1","true","yes")
    use_pair_te = str(os.getenv("USE_PAIR_TE", "0")).lower() in ("1","true","yes")
    use_wls     = str(os.getenv("USE_HETERO_W", "0")).lower() in ("1","true","yes")
    use_mono    = str(os.getenv("USE_MONO_CUR", "0")).lower() in ("1","true","yes")
    rm_trainonly= str(os.getenv("USE_RM_TRAINONLY", "0")).lower() in ("1","true","yes")
    ...
    # 라벨/예측 스멜 제거
    def _hard_leak_guard(cols: list[str]) -> list[str]:
        bad_kw = ("target", "label", "pred", "oof", "hat", "te_")
    ...
    if anchor_path.exists():
        test_plates = set(str(x) for x in joblib.load(anchor_path))
    ...
    # (선택) RM 집계 Train-only 강제: RM_* 열 제거로 누수 위험 회피
    if rm_trainonly:
        rm_cols = [c for c in tr_df_raw.columns if c.startswith("RM_")]
        tr_df_raw = tr_df_raw.drop(columns=rm_cols, errors="ignore")
        te_df_raw = te_df_raw.drop(columns=rm_cols, errors="ignore")

    # 피처 프레임 구성 (EXCLUDE 제거 후 스멜 이름 제거)
    ...
    feat_cols_all = _hard_leak_guard(sorted(set(tr_base.columns)))
    tr_feats = tr_base[feat_cols_all].copy()
    te_feats = te_base[feat_cols_all].copy()

    # 숫자 행렬 + 중앙값(train) 대치
    Xtr_df, med, base_cols = _ensure_numeric_X(tr_feats, EXCLUDE_FROM_X)
    Xte_df, _,  _         = _ensure_numeric_X(te_feats, EXCLUDE_FROM_X, base_cols=base_cols, medians=med)

    y_tr = tr_df_raw[TARGET_COL].values
    y_te = te_df_raw[TARGET_COL].values
    y_te_true = y_te.copy()

    # Δ-타깃 (학습 타깃을 Δ로 바꾸고, 예측 후 복원)
    if use_delta:
        y_tr = y_tr - tr_df_raw[CUR_WARP].values
        y_te = y_te - te_df_raw[CUR_WARP].values

    # train-only 이상치 가드 (특징 clip + 타깃 윈저라이즈)
    q_low, q_high = 0.01, 0.99
    bounds = {}
    for c in Xtr_df.columns:
        lo, hi = np.quantile(Xtr_df[c].values, [q_low, q_high])
        bounds[c] = (float(lo), float(hi))
    y_lo, y_hi = np.quantile(y_tr, [0.005, 0.995])

    keep = (y_tr >= y_lo) & (y_tr <= y_hi)
    ...
    # (옵션) Pair Δ-TE 추가
    if use_pair_te:
        te_tr, te_map, te_def = _oof_pair_te(tr_df_raw, y_tr if not use_delta else (y_tr + 0), use_delta=use_delta)
        Xtr_df["PAIR_TE"] = te_tr
        # test는 글로벌 맵 주입
        pair_te = _make_pair_id(te_df_raw).map(lambda p: te_map.get(p, te_def)).astype(float)
        Xte_df["PAIR_TE"] = pair_te.values
    ...
    if algorithm == "RandomForest":
        ...
        if use_mono and CUR_WARP in sel_cols:
            # 선택열 순서에 맞춰 단조 제약 설정 (CUR_WARP만 +1)
            constraints = [0]*len(sel_cols)
            constraints[sel_cols.index(CUR_WARP)] = 1
        model = XGBRegressor(
            random_state=seed, tree_method="hist", n_jobs=0,
            n_estimators=500, learning_rate=0.05,
            max_depth=6, subsample=0.8, colsample_bytree=0.8,
            reg_lambda=2.0, reg_alpha=0.5, min_child_weight=10,
            **({"monotone_constraints": tuple(constraints)} if constraints else {})
        )
        fit_kwargs = {}
    ...
    sw = None
    if use_wls:
        sw = _pair_weights(tr_df_raw, y_tr if not use_delta else (y_tr + 0), use_delta=use_delta)
    ...
    joblib.dump(list(Xtr_df.columns), MODELS_DIR / f"{prefix}_selector_fit_cols_tplus1.pkl")  # ★학습 입력 전체열
...
def _predict_next_from_row(row: pd.Series, bundle: dict) -> float:
    """
    transform()의 피처 이름 검사 에러를 피하기 위해 학습 당시 입력열(fit_cols) 순서를
    그대로 재구성 후 selector 마스크로 수동 슬라이싱.
    """
