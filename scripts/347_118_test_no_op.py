...
USE_DELTA_TARGET = _env_on("USE_DELTA_TARGET", "1")
USE_PAIR_TE      = _env_on("USE_PAIR_TE", "1")
USE_HETERO_W     = _env_on("USE_HETERO_W", "0")
USE_MONO_CUR     = _env_on("USE_MONO_CUR", "0")
USE_RM_TRAINONLY = _env_on("USE_RM_TRAINONLY", "0")

USE_LABEL_CLEAN  = _env_on("USE_LABEL_CLEAN", "1")   # (A) 타깃/샘플 정제: 기본 ON
USE_PASS_EXPERT  = _env_on("USE_PASS_EXPERT", "1")   # (C) pass별 expert SVR: 기본 ON
...
def load_and_merge_data_tplus1() -> pd.DataFrame:
    ...
    # 타깃 생성 (warping_index_target = t+1)
    ...
    # ---- PASS 연속성(+1) 강제: 불연속 전이 제거 (A: 구조 정제 핵심) ----
    merged_fm["_next_pass"] = merged_fm.groupby(PLATE_COL)[PASS_COL].shift(-1)
    diff = merged_fm["_next_pass"] - merged_fm[PASS_COL]
    nonconsec = diff.notna() & (diff != 1)
    n_bad = int(nonconsec.sum())
    if n_bad > 0:
        print(f"[WARN] PASS(t)→PASS(t+1) 불연속 전이 {n_bad}행: 타깃 제거 후 드롭")
        merged_fm.loc[nonconsec, TARGET_COL] = np.nan
    merged_fm = merged_fm.drop(columns=["_next_pass"])

    before = len(merged_fm)
    merged_fm = merged_fm.dropna(subset=[TARGET_COL]).reset_index(drop=True)
    print(f"Target shift(-1) & PASS 연속성 필터: {before} → {len(merged_fm)} (삭제 {before-len(merged_fm)})")
...
def add_sequence_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    plate 단위 짧은 시계열에서만 쓰는 안전한 FE:
    - lag1_warp: 직전 pass warping_index_current_pass
    - d1_warp: 현재 - 직전
    - warp_roll_mean3 / warp_roll_std3: 길이 3 이동 평균/표준편차
    """
    if CUR_WARP not in df.columns:
        return df

    d = df.copy()
    idx_order = d.index

    d_sorted = d.sort_values([PLATE_COL, PASS_COL])
    g = d_sorted.groupby(PLATE_COL, sort=False)

    d_sorted["lag1_warp"] = g[CUR_WARP].shift(1)
    d_sorted["d1_warp"] = d_sorted[CUR_WARP] - d_sorted["lag1_warp"]

    roll = g[CUR_WARP].rolling(window=3, min_periods=1)
    d_sorted["warp_roll_mean3"] = roll.mean().reset_index(level=0, drop=True)
    d_sorted["warp_roll_std3"]  = roll.std().reset_index(level=0, drop=True)

    # 원래 행 순서로 복원
    d_sorted = d_sorted.loc[idx_order]
    return d_sorted
...
def _fit_passwise_svm_experts(
    Xtr_sel: np.ndarray,
    y_tr_model: np.ndarray,
    pass_tr: np.ndarray,
    Xte_sel: np.ndarray,
    pass_te: np.ndarray,
    global_model,                         # 이미 학습된 글로벌 SVR 파이프라인
    min_samples_per_pass: int = 40,
):
    """
    (C) pass별 expert SVM:
    - 기본적으로 global_model 예측을 baseline으로 깔고
    - 각 PASS 별로 충분히 샘플이 있을 때 그 subset만 따로 SVR을 다시 학습해서
      해당 PASS의 train/test 예측을 덮어쓴다.
    - 최종적으로 모델 타깃 공간의 y_tr_pred_model, y_te_pred_model을 반환.
    """
    y_tr_pred = global_model.predict(Xtr_sel)
    y_te_pred = global_model.predict(Xte_sel)

    unique_passes = np.unique(pass_tr)
    print(f"[PASS_EXPERT] 후보 PASS 목록: {unique_passes}")
    ...
    for p in unique_passes:
        ...
        if n_tr < min_samples_per_pass:
            print(f"[PASS_EXPERT] PASS={p} train 샘플 {n_tr}개 < {min_samples_per_pass} → global 모델 유지")
            continue

        # 해당 PASS에 대해 로컬 SVR 파이프라인 학습
        local_model = Pipeline([
            ("scale", StandardScaler(with_mean=True, with_std=True)),
            ("reg",   SVR(kernel="rbf", C=10.0, epsilon=0.1)),
        ])

        Xtr_p = Xtr_sel[mask_tr]
        ytr_p = y_tr_model[mask_tr]
        Xte_p = Xte_sel[mask_te]

        local_model.fit(Xtr_p, ytr_p)
        y_tr_pred[mask_tr] = local_model.predict(Xtr_p)

        if mask_te.any():
            y_te_pred[mask_te] = local_model.predict(Xte_p)

        print(f"[PASS_EXPERT] PASS={p} local SVR 학습 완료 (train {n_tr}행)")
...
def train_model(df: pd.DataFrame,
    ...
    # ---------------- (A-2) Train 타깃 quantile 기반 label cleaning ----------------
    y_tr_orig = y_tr.copy()
    y_te_orig = y_te.copy()
    ...
    if USE_LABEL_CLEAN:
        q_low, q_high = 0.005, 0.995   # 필요하면 ENV로 빼도 됨
        y_lo = np.quantile(y_tr_orig, q_low)
        y_hi = np.quantile(y_tr_orig, q_high)
        clean_mask = (y_tr_orig >= y_lo) & (y_tr_orig <= y_hi)
        dropped = int((~clean_mask).sum())
        if dropped > 0:
            print(f"[LABEL_CLEAN] train 타깃 {q_low*100:.1f}~{q_high*100:.1f} quantile 밖 {dropped}행 제거")
            X_tr_raw = X_tr_raw.loc[clean_mask].reset_index(drop=True)
            y_tr = y_tr[clean_mask]
            y_tr_orig = y_tr_orig[clean_mask]
            cur_tr = cur_tr[clean_mask]
            tr_df_raw = tr_df_raw.loc[clean_mask].reset_index(drop=True)
    else:
        print("[LABEL_CLEAN] 사용 안 함 (USE_LABEL_CLEAN=0)")

    # -------- (B/C 공통 준비) Δ‑target & pair Δ TE 준비 --------
    # 평가용 진짜 타깃은 y_tr_orig / y_te_orig 그대로 유지
    if USE_DELTA_TARGET:
        y_tr_model = y_tr_orig - cur_tr
        print("[DELTA_TARGET] y_model = y(t+1) - warping_index_current_pass(t)")
    else:
        y_tr_model = y_tr_orig.copy()
        print("[DELTA_TARGET] 사용 안 함 (원 타깃 그대로)")

    # pair Δ target encoding: t→t+1 전이쌍별 평균 Δ (OOF, EB 수축)
    if USE_PAIR_TE:
        te_oof, te_global_means, te_global_default = _oof_pair_te(
            tr_df_raw, y_tr_model, use_delta=False, n_splits=5
        )
        pair_test = _make_pair_id(te_df_raw).values
        te_test = np.array([te_global_means.get(p, te_global_default) for p in pair_test])
        X_tr_raw = X_tr_raw.copy()
        X_te_raw = X_te_raw.copy()
        X_tr_raw["pair_delta_te"] = te_oof
        X_te_raw["pair_delta_te"] = te_test
        print("[PAIR_TE] pair_delta_te 특성 추가 완료")
    else:
        print("[PAIR_TE] 사용 안 함 (USE_PAIR_TE=0)")
    ...
    selector.fit(X_tr.values, y_tr_model)  # 학습 타깃 기준으로 feature 선택
    ...
    # 6) 학습/평가 (먼저 글로벌 모델 학습: 타깃은 y_tr_model)
    model.fit(Xtr_sel, y_tr_model)
    y_tr_pred_model = model.predict(Xtr_sel)
    y_te_pred_model = model.predict(Xte_sel)

    # (C) PASS_EXPERT: pass별 로컬 SVR로 글로벌 예측을 덮어쓰기
    if USE_PASS_EXPERT and algorithm == "SVM":
        pass_tr = tr_df_raw[PASS_COL].values
        pass_te = te_df_raw[PASS_COL].values
        y_tr_pred_model, y_te_pred_model = _fit_passwise_svm_experts(
            Xtr_sel, y_tr_model, pass_tr,
            Xte_sel, pass_te,
            global_model=model,
            min_samples_per_pass=40,
        )
        print("[PASS_EXPERT] pass별 expert SVR 적용 완료")
    else:
        print("[PASS_EXPERT] 사용 안 함 (USE_PASS_EXPERT=0 또는 algo!=SVM)")

    # Δ‑target을 썼다면 원 스케일로 복원
    if USE_DELTA_TARGET:
        y_tr_pred = cur_tr + y_tr_pred_model
        y_te_pred = cur_te + y_te_pred_model
    else:
        y_tr_pred = y_tr_pred_model
        y_te_pred = y_te_pred_model

    # 최종 평가는 항상 원 타깃 스케일(y_tr_orig / y_te_orig) 기준
    res = {
        "train_r2":   float(r2_score(y_tr_orig, y_tr_pred)),
        "test_r2":    float(r2_score(y_te_orig, y_te_pred)),
        "train_rmse": float(np.sqrt(mean_squared_error(y_tr_orig, y_tr_pred))),
        "test_rmse":  float(np.sqrt(mean_squared_error(y_te_orig, y_te_pred))),
        "train_samples": int(len(y_tr_orig)),
        "test_samples":  int(len(y_te_orig)),
...
def main():
    ...
    # 요구사항: 파일명 고정
    summary_path = REPORTS_SUMMARY_DIR / f"{experiment_id}_118_test_no_op_summary.txt"
    archived_script_path = REPORTS_CODE_DIR / f"{experiment_id}_118_test_no_op.py"
