...
SELECTK_K = int(_os.environ.get("SELECTK_K", "90"))  # 과적합 완화: 120→90
...
def _build_xgb(robust=False, monotone_index=None, n_features=None, seed=SEED):
    """
    S0 과적합 완화용 강한 정규화 기본값.
    - max_depth ↓, min_child_weight ↑, subsample/colsample_bytree ↓
    - L1(α)/L2(λ) ↑, gamma ↑
    - eval_set/early_stopping은 fit에 넘기지 않음(버전 호환 + 장황 로그 차단)
    """
    params = dict(
        random_state=seed,
        tree_method="hist",
        # n_estimators는 _fit_predict_model 에서 소형 그리드로 선택
        n_estimators=350,
        learning_rate=0.05,
        max_depth=4,
        min_child_weight=20,
