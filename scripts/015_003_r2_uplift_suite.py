...
def train_single_model(train_df, test_df, direction_key: str, flags: set, algo_name: str):
    """
    누수 차단형 단일 모델 학습/예측
    - 모든 통계/선택/적합은 train에서만 수행
    - test에는 train에서 만든 메타(열순서/중앙값/선택자/매핑통계)만 적용
    - plate 그룹 교집합 금지 검사 포함
    """
    import numpy as np
    from sklearn.metrics import r2_score, mean_squared_error
    ...
    # ---- [Guard 0] plate 교집합 금지 ----
    tr_plates = set(tr[PLATE_COL].astype(str).unique())
    te_plates = set(te[PLATE_COL].astype(str).unique())
    overlap = tr_plates.intersection(te_plates)
    if overlap:
        raise RuntimeError(f"[LEAK-GUARD] Train/Test plate overlap detected: {sorted(list(overlap))[:5]} ...")

    # ---- TE2: Δ-타깃 인코딩 (OOF train → test full-train 매핑) ----
    if "PAIR_TE" in flags:
        tr, te = add_pair_delta_te_feature(tr, te, n0=PAIR_EB_N0)  # train OOF만, test는 full-train 매핑
    ...
    if ("HETERO_W" in flags) or ("IMP_WEIGHT" in flags):
        ...
        if "HETERO_W" in flags:
            w *= compute_oof_pair_stats_and_weights(tr, te)["w_tr"]  # train OOF
        if "IMP_WEIGHT" in flags:
            # IW_MODE="oof"일 때만 사용(누수 안전); "test" 모드는 차단
            if IW_MODE != "oof":
                raise RuntimeError("[LEAK-GUARD] Importance weighting must be OOF-only (IW_MODE='oof').")
            iw = compute_importance_weights(tr, te)
            if len(iw) == len(tr):
                w *= iw
        sample_weight = w / (np.mean(w) + 1e-8)

    # ---- 특성 구성/결측 대체는 train 기준 ----
    Xtr_df, med, cols = _ensure_numeric_X(tr)   # TARGET/PLATE/PASS/MONTH 등 제외됨
    if TARGET_COL in Xtr_df.columns:
        raise RuntimeError("[LEAK-GUARD] TARGET_COL leaked into Xtr_df.")
    # test는 train 열순서/중앙값으로 정렬/대체
    Xte_df = te[cols].copy()
    for c in cols:
        if c not in Xte_df.columns:
            Xte_df[c] = np.nan
    Xte_df = Xte_df[cols].fillna(med)

    # ---- SelectKBest: train으로만 적합 ----
    chosen, Xtr, Xte = select_features(Xtr_df, tr["_target"].values, Xte_df, mandatory=mandatory, k=SELECTK_K)

    # ---- 학습/예측 (단조/로버스트/배깅 반영 + 가중치) ----
    mdl, pte = _fit_predict_model(Xtr, tr["_target"].values, Xte, flags, algo_name, chosen, sample_weight=sample_weight)

    # ---- Train 예측(리포트용) ----
    ptr = mdl.predict(Xtr)

    # ---- Δ 복원 (test는 test의 CUR_WARP만 더함; 라벨 접근 없음) ----
    if "DELTA_TARGET" in flags:
    ...
    # ---- (옵션) 미니 퍼뮤테이션 체크: 환경변수로 켤 수 있음 ----
    if os.environ.get("LEAK_PERM_CHECK", "0") == "1":
        yperm = tr["_target"].sample(frac=1.0, random_state=SEED).values  # plate 내 셔플은 별도 구현 가능
        _mdl, _pte = _fit_predict_model(Xtr, yperm, Xte, flags, algo_name, chosen, sample_weight=sample_weight)
        if "DELTA_TARGET" in flags:
            _pte = te[CUR_WARP].values + _pte
        r2_perm = float(r2_score(te[TARGET_COL].values, _pte))
        if r2_perm > 0.05:  # 0 근처여야 함(완화 임계치)
            raise RuntimeError(f"[LEAK-GUARD] Permutation test R²={r2_perm:.3f} (>0.05). Possible leakage.")
    ...
    return dict(bundle=bundle, r2_tr=r2_tr, r2_te=r2_te, rmse_tr=rmse_tr, rmse_te=rmse_te,
                pred_tr=ptr, pred_te=pte, keys_tr=k_tr, keys_te=k_te)
...
def main():
    ...
    # 2) 앵커 정책(공정 비교 기본) ----------------------------------------
    #   - 기본: RUN 전체 공통 앵커(권장)
    #   - 옵션: ANCHOR_PER_STEP=1 이면 스텝별 앵커(과거 동작 재현)
    #   - 옵션: ANCHOR_FROM_CROSS=1 이면 003에서 저장된 테스트 판을 강제 사용
    per_step_anchor = (os.environ.get("ANCHOR_PER_STEP", "0") == "1")
    force_cross     = (os.environ.get("ANCHOR_FROM_CROSS", "0") == "1")

    # 3) 타깃 퍼뮤테이션(누수 자가진단) -----------------------------------
    #   - PERMUTE_TARGET=1 이면 plate 내에서 target을 무작위 섞은 데이터로 추가 평가
    do_permute = (os.environ.get("PERMUTE_TARGET", "0") == "1")
    if do_permute:
        shuffled = final_df.copy()
        rng = np.random.RandomState(SEED)
        shuffled[TARGET_COL] = shuffled.groupby(PLATE_COL)[TARGET_COL] \
                                       .transform(lambda s: s.sample(frac=1.0, random_state=rng.randint(1, 10**9)).values)
        e2x_raw_perm, x2e_raw_perm = split_e2x_x2e(shuffled)
    # -------------------------------------------------------------------

    # 4) 실행 순서: 항상 S0 먼저 → 다른 모드(동일 앵커에서 공정 비교)
    #    (S0가 없으면 주어진 순서대로 실행)
    keys = list(recipes.keys())
    if "S0" in recipes:
        keys = ["S0"] + [k for k in keys if k != "S0"]
    ...
    # 6) RUN 공통 앵커 이름(기본) / 스텝별 앵커(옵션)
    def _anchor_name(flags, key):
        if force_cross:
            # split_by_anchor() 내부에서 003 테스트판을 자동 로드(환경 신호)하도록 함
            os.environ["ANCHOR_FROM_CROSS"] = "1"  # 안전히 주입
            return f"{RUN_NAME}__CROSS__anchor"
        base = f"{RUN_NAME}__{'STRAT' if 'STRAT_SPLIT' in flags else 'BASE'}__anchor"
        return f"{base}__{key}" if per_step_anchor else base

    # 7) 스텝 루프
    s0_avg = None
    for key in keys:
        flags = set(recipes[key])
        split_anchor = _anchor_name(flags, key)

        print("\n" + "="*80)
        print(f"[RUN] {key} | flags={sorted(list(flags))} | anchor={split_anchor}")
        print("="*80)
        ...
        # Strict roll-forward R²(교집합 Test 판만)
        e2x_pred_map = { (p,pa): float(v) for (p,pa), v in zip(e2x_res["keys_te"], e2x_res["pred_te"]) } if "keys_te" in e2x_res else {}
        x2e_pred_map = { (p,pa): float(v) for (p,pa), v in zip(x2e_res["keys_te"], x2e_res["pred_te"]) } if "keys_te" in x2e_res else {}
        rf_strict = strict_rollforward_r2(
            final_df,
            set(e2x_res.get("test_plates", [])),
            set(x2e_res.get("test_plates", [])),
        ...
        perm_avg = np.nan
        if do_permute:
            e2x_perm = run_one_direction(e2x_raw_perm, "E2X", flags, split_anchor)
            x2e_perm = run_one_direction(x2e_raw_perm, "X2E", flags, split_anchor)
            perm_avg = float(np.mean([e2x_perm["r2_te"], x2e_perm["r2_te"]]))
            if perm_avg > 0.05:
                print(f"[LEAK-ALERT] PERMUTE avg R²={perm_avg:.4f} (>0). 파이프라인 누수 의심.")

        # 결과 파일(스텝)
        steps_csv   = csv_dir / f"{exp_id}_{RUN_NAME}_{key}_steps.csv"
        summary_txt = (REPORTS_DIR / "summaries") / f"{exp_id}_{RUN_NAME}_{key}_summary.txt"

        pd.DataFrame([{
            "step": key,
            "E2X_test_r2": e2x_res["r2_te"], "X2E_test_r2": x2e_res["r2_te"], "Avg_test_r2": avg_r2,
            "E2X_train_r2": e2x_res["r2_tr"], "X2E_train_r2": x2e_res["r2_tr"],
            "RF_strict": rf_strict,
            "Permute_AvgR2": perm_avg,
            "anchor": split_anchor
        ...
        lines.append("[모델 성능 - R2]"); lines.append(fmt("E2X", e2x_res)); lines.append(fmt("X2E", x2e_res))
        lines.append(f"\n[평균 Test R2] {avg_r2:.4f}")
        lines.append(f"[Strict Roll-forward R²] {rf_strict:.4f}")
        if do_permute: lines.append(f"[Permutation Avg R²] {perm_avg:.4f}")
        lines.append("="*72)
        # 시동어 설명(간단)
        lines.append("\n[시동어 설명]")
        if "+" in key:
        ...
        lines.append(f"\n[산출물]"); lines.append(f"- 단계별 결과 CSV: {steps_csv}"); lines.append(f"- 코드 스냅샷: {code_snap}")
        summary_txt.write_text("\n".join(lines), encoding="utf-8")
        print(f"✓ 저장: {steps_csv}"); print(f"✓ 저장: {summary_txt}")

        # 리더보드 누적
        if s0_avg is None and key == "S0": s0_avg = avg_r2
        delta_vs_s0 = (avg_r2 - s0_avg) if (s0_avg is not None) else np.nan
    ...
    # 8) 리더보드 저장 및 출력
    lb = pd.DataFrame(leaderboard_rows).sort_values("avg_test_r2", ascending=False)
    lb_path = csv_dir / f"{exp_id}_{RUN_NAME}_leaderboard.csv"
    lb.to_csv(lb_path, index=False)
    print("\n[Leaderboard — 높은 Avg Test R² 순]"); print(lb.to_string(index=False))
    print(f"\n✓ 리더보드 저장: {lb_path}")
