"""
032_test.py 기반 코드 수정 (leak-hard-guard, optuna-free, auto-leak-audit, anchor-reuse)

- RUN_ONLY / RUN_WITH 프리셋 제공
- 하드 라벨 누수 차단(HARD_LEAK_GUARD)
- FIXED_ANCHOR_NAME 제공 시, permute off→on에도 동일 분할 재사용
- X2E 강화(A: MOE, B: YJ, C: Residual), TE2/W2/IW OOF-안전
"""

# ========================= [ USER CONFIG ] =========================
import os as _os

RUN_NAME = _os.environ.get("RUN_NAME", "032_test")

# 재현·탐색 기본 프리셋 (필요 시 RUN_ONLY_EXPR / RUN_WITH_EXPR로 오버라이드)
RUN_ONLY = []  # 기본적으로 RUN_WITH 프리셋 사용 (RUN_ONLY_EXPR로 오버라이드)
RUN_WITH = [
    "S0",                                # 기준 baseline
    #"S0+TE2+W2+M2",                      # Δ TE + 이분산 + CUR_WARP 단조
    #"S0+TE2+W2+M2+BAG",                  # + bagging
    #"S0+L1+DF1+TE2+W2+M2+BAG",           # 시퀀스/정규화/Δ/TE2/W2 + 단조 + bagging
    #"S0+TE2+T2+W2+M2+BAG",               # Δ‑Target + TE2/W2 + 단조 + bagging
    #"S0+TE2+X2E_RES+M2+W2",              # X2E Residual 2-stage (안정형)
    #"S0+TE2+X2E_RES+M2+W2+BAG",          # X2E Residual + bagging
    #"S0+TE2+X2E_YJ+M2+W2+BAG",           # X2E YJ + bagging
]

# ★ 고정 알고리즘 (두 방향 모두 XGB 사용으로 단순화; 필요 시 E2X/X2E 다르게 설정 가능)
ALGO_FIXED = {"E2X": "XGBoost", "X2E": "XGBoost"}

# 공통 설정
SEED = int(_os.environ.get("SEED", "42"))
TEST_SIZE = float(_os.environ.get("TEST_SIZE", "0.2"))
CV_FOLDS = int(_os.environ.get("CV_FOLDS", "3"))
SELECTK_K = int(_os.environ.get("SELECTK_K", "40"))  # 과적합 완화: 120→90->40
PAIR_EB_N0 = int(_os.environ.get("PAIR_EB_N0", "50"))
WEIGHT_CLIP = (float(_os.environ.get("WCLIP_LO","0.25")), float(_os.environ.get("WCLIP_HI","3.0")))
STRAT_TRY = int(_os.environ.get("STRAT_TRY", "25"))
BAG_K = int(_os.environ.get("BAG_K", "3"))           # 러닝타임 고려 K=3
DO_STRICT_ROLLFORWARD = (_os.environ.get("DO_STRICT_ROLLFORWARD","0")=="1")
IW_MODE = "oof"

# ---- 분할/감사/가드 플래그(환경변수) ----
def _env_on(n: str, d="0") -> bool:
    return str(_os.environ.get(n, d)).strip().lower() in {"1","true","yes","y"}
...
HARD_LEAK_GUARD   = _env_on("HARD_LEAK_GUARD","1")  # 기본 ON
FIXED_ANCHOR_NAME = _os.environ.get("FIXED_ANCHOR_NAME", "").strip()  # 있으면 무조건 이 이름으로 분할 재사용

# X2E 강화(전략 A/B/C)
X2E_EXPERTS   = _env_on("X2E_EXPERTS","0")   # A: Pair experts
X2E_RESIDUAL  = _env_on("X2E_RESIDUAL","0")  # C: Residual 2-stage
X2E_MONO      = _env_on("X2E_MONO","0")      # CUR_WARP 단조(+1)
# ================================================================
...
LOG_DIR = REPORTS_DIR / "logs"; LOG_DIR.mkdir(parents=True, exist_ok=True)
...
ALIAS_DESC = {
    "S0":  "Baseline(SelectK 90, plate-Group split, E2X=XGB/X2E=XGB)",
    ...
    "S1":  "Stratified Group Split (pair 분포 유사화)",
    ...
    "SEG": "Phase Segmentation (MoE placeholder)",
    "IW":  "Importance Weighting (Train OOF)",
    "BAG":  "Bagging(K=3)",
    "X2E_MOE": "X2E: Pair-wise Experts with fallback",
    "X2E_YJ":  "X2E: Train-only Yeo-Johnson target transform",
    "X2E_RES": "X2E: 2-stage residual learning (OOF-safe)",
}
...
def _sanitize_feature_columns(df: pd.DataFrame) -> pd.DataFrame:
    """라벨/타깃/permute 플래그 등 이름 패턴으로 **완전 제거**."""
    if not HARD_LEAK_GUARD:
        return df
    pat = re.compile(r"(?i)(^|_)(target|label|permute)\b|(^y$)|(^y_)|(_y$)")
    drops = [c for c in df.columns if pat.search(c)]
    if drops:
        GLOBAL_DIAG["dropped_cols_by_guard"].extend(drops)
...
def _ensure_numeric_X(df: pd.DataFrame):
    ...
    # 하드 가드: 이름 패턴으로 컬럼 제거
    X_all = _sanitize_feature_columns(X_all)
...
def load_and_merge_tplus1():
    ...
    n_bad = int(nonconsec.sum()); GLOBAL_DIAG["nonconsec_dropped"] = n_bad
    if n_bad > 0:
...
def compute_importance_weights(train_df: pd.DataFrame, test_df: pd.DataFrame):
    assert IW_MODE == "oof", "Only OOF IW is allowed."
    tr = train_df.copy()
...
def split_by_anchor(df: pd.DataFrame, direction_key: str, use_strat: bool, anchor_name: str):
    # 1) 고정 앵커가 있으면 그걸 최우선 사용
    fixed_anchor = os.environ.get("FIXED_ANCHOR_NAME")
    if fixed_anchor:
        anchor_file = MODELS_DIR / f"{fixed_anchor}__{direction_key}_test_plates.pkl"
    else:
        anchor_file = MODELS_DIR / f"{anchor_name}__{direction_key}_test_plates.pkl"

    # 2) 003 크로스 앵커 강제(선택)
    if ANCHOR_FROM_CROSS:
        cross_anchor = MODELS_DIR / ("e2x_test_plates_tplus1.pkl" if direction_key=="E2X"
                                     else "x2e_test_plates_tplus1.pkl")
    ...
    # Stratified by pair histogram L1
    pairs = _pair_str(df)
    pair_vals = sorted(pairs.unique().tolist())
    best = (1e9, None, None)
    rng = np.random.RandomState(SEED)
    for _ in range(STRAT_TRY):
        seed = int(rng.randint(0, 1_000_000_000))
        gss = GroupShuffleSplit(n_splits=1, test_size=TEST_SIZE, random_state=seed)
        tr_i, te_i = next(gss.split(df.drop(columns=[TARGET_COL]), df[TARGET_COL],
                                    groups=df[PLATE_COL].astype(str).values))
...
def select_features(Xtr_df: pd.DataFrame, ytr: np.ndarray,
    ...
    # 1) 사전 정화
    Xtr_df = _sanitize_feature_columns(Xtr_df)
    Xte_df = _sanitize_feature_columns(Xte_df)
    ...
    fill_med = Xtr.median(numeric_only=True)
    Xtr = Xtr.fillna(fill_med)
    Xte = Xte.fillna(fill_med)
...
def _fit_xgb_safe(model, Xfit, yfit, *, sample_weight=None, Xval=None, yval=None, early_rounds: int = 50):
    """
    XGBoost 버전 차이에 안전하게 대응하는 fit 래퍼.
    - early_stopping_rounds 지원 → 그대로 사용
    - callbacks 지원 → EarlyStopping(rounds=early_rounds) 사용
    - 둘 다 불가 → n_estimators를 자동 축소(최대 300) 후 일반 fit
    """
    # 시그니처 점검
    try:
        sig = signature(model.fit)
        has_es = ("early_stopping_rounds" in sig.parameters)
        has_callbacks = ("callbacks" in sig.parameters)
        has_eval_set = ("eval_set" in sig.parameters)
        has_sample_weight = ("sample_weight" in sig.parameters)
    except Exception:
        has_es = has_callbacks = has_eval_set = has_sample_weight = False

    # 공통 kwargs
    fit_kwargs = {}
    if has_sample_weight and sample_weight is not None:
        fit_kwargs["sample_weight"] = sample_weight

    # 조기중단 경로
    if Xval is not None and yval is not None and has_eval_set:
        fit_kwargs["eval_set"] = [(Xval, yval)]

        if has_es:
            # 구버전/일부 1.x/2.x에서 정상 작동
            fit_kwargs["early_stopping_rounds"] = early_rounds
            model.fit(Xfit, yfit, **fit_kwargs)
            return model

        elif has_callbacks:
            # 일부 2.x 계열: callbacks 방식만 허용
            try:
                from xgboost.callback import EarlyStopping
                fit_kwargs["callbacks"] = [EarlyStopping(rounds=early_rounds, save_best=True)]
            except Exception:
                # 콜백 import 실패 시 일반 학습으로 폴백
                pass
            model.fit(Xfit, yfit, **fit_kwargs)
            return model

    # 조기중단 불가 → 안전 가드: 트리 개수 축소 후 일반 fit
    try:
        ne = int(model.get_params().get("n_estimators", 600))
        if ne > 300:
...
def _build_xgb(robust=False, monotone_index=None, n_features=None, seed=SEED):
    """
    좀 더 보수적인 기본값 (small data + group split용)
    - max_depth=3, min_child_weight=40으로 용량 축소
    ...
    params = dict(
        random_state=seed,
        tree_method="hist",
        n_estimators=200,          # 기본값 (실제 사용은 _fit_predict_model에서 다시 세팅)
        learning_rate=0.05,
        max_depth=3,               # 4 → 3
        min_child_weight=40,       # 20 → 40
        subsample=0.50,            # 0.60 → 0.50
        colsample_bytree=0.50,     # 0.60 → 0.50
        colsample_bylevel=0.80,
        gamma=2.0,                 # 1.0 → 2.0
        reg_lambda=20.0,           # 12.0 → 20.0
        reg_alpha=4.0,             # 2.0 → 4.0
        n_jobs=0,
        eval_metric="rmse",
    )
    if (monotone_index is not None) and (n_features is not None):
        v = [0] * n_features
        if 0 <= monotone_index < n_features:
            v[monotone_index] = 1   # CUR_WARP 단조(+1)
...
def _fit_predict_model(Xtr, ytr, Xte, flags, algo_name, chosen_names,
    ...
    """
    - 내부 홀드아웃(GroupShuffleSplit)에서 소형 그리드의 n_estimators를 빠르게 스캔 → 최적값 선택
    - eval_set/early_stopping_rounds 인자는 사용하지 않음 (구버전 xgboost TypeError 방지 + 과한 로그 차단)
    - 최적 n_estimators로 전체 train 재학습 후 test 예측 반환
    """
    # monotone(+1) on CUR_WARP (M2일 때만)
    mono_idx = None
    if ("MONO_CUR" in flags) and (algo_name == "XGBoost") and (CUR_WARP in chosen_names):
        mono_idx = chosen_names.index(CUR_WARP)

    def make_one(seed):
        if algo_name == "XGBoost":
            return _build_xgb(("ROBUST_LOSS" in flags), monotone_index=mono_idx,
                              n_features=len(chosen_names), seed=seed)
        else:
            return _build_rf(seed=seed)

    # 내부 홀드아웃 (plate group 보존)
    ...
    if algo_name == "XGBoost":
        ...
        for n_est in N_CAND:
            m = make_one(SEED)
            m.set_params(n_estimators=int(n_est))
            # eval_set, early_stopping_rounds 미사용 (구버전 호환)
            if sw_fit is None:
                m.fit(Xfit, yfit)
            else:
                m.fit(Xfit, yfit, sample_weight=sw_fit)
            pred_val = m.predict(Xval)
            rmse = np.sqrt(mean_squared_error(yval, pred_val))
            if rmse < best_rmse:
                best_rmse = rmse
                best_n = int(n_est)

        # 최적 라운드로 전체 train 재학습 → test 예측
        m_final = make_one(SEED)
        m_final.set_params(n_estimators=best_n)
        if sample_weight is None:
            m_final.fit(Xtr, ytr)
        else:
            m_final.fit(Xtr, ytr, sample_weight=sample_weight)
        yhat = m_final.predict(Xte)
        return m_final, yhat

    # --- RandomForest 등 다른 알고리즘 ---
    m = make_one(SEED)
    if sw_fit is None:
        m.fit(Xfit, yfit)
        m.fit(Xtr, ytr)  # full 재학습
    else:
        m.fit(Xfit, yfit, sample_weight=sw_fit)
...
def _fit_predict_model_ensemble(Xtr, ytr, Xte, flags, algo_name, chosen_names,
                                sample_weight=None, groups: np.ndarray | None = None,
                                n_bags: int = BAG_K):
    """
    BAGGING 플래그가 켜져 있으면 bootstrap bagging으로 여러 모델을 학습해 예측을 평균한다.
    - train_single_model에서만 사용된다.
    - 데이터를 test와 섞지 않고, train 안에서만 bootstrap 샘플링하므로 데이터 누수는 없다.
    """
    # bagging이 아니면 기존 단일 모델 경로를 그대로 사용
    if ("BAGGING" not in flags) or (n_bags is None) or (n_bags <= 1):
        model, yhat_te = _fit_predict_model(
            Xtr, ytr, Xte, flags, algo_name, chosen_names,
            sample_weight=sample_weight, groups=groups,
        )
        yhat_tr = model.predict(Xtr)
        return [model], yhat_tr, yhat_te
    ...
    for b in range(int(n_bags)):
        ...
        m_b, yhat_te_b = _fit_predict_model(
            Xtr_b, ytr_b, Xte, flags, algo_name, chosen_names,
            sample_weight=sw_b, groups=grp_b,
        )
        te_preds.append(yhat_te_b.reshape(1, -1))
        tr_preds.append(m_b.predict(Xtr).reshape(1, -1))
        models.append(m_b)

    yhat_te = np.mean(np.vstack(te_preds), axis=0)
    yhat_tr = np.mean(np.vstack(tr_preds), axis=0)
    return models, yhat_tr, yhat_te
...
def train_x2e_boosted(train_df: pd.DataFrame, test_df: pd.DataFrame, flags: set):
    ...
    if use_yj:
        pt = fit_yj_transformer(y_raw_tr)
        y_tr = yj_transform(pt, y_raw_tr)
    else:
        pt = None
        y_tr = y_raw_tr
...
def train_single_model(train_df, test_df, direction_key: str, flags: set, algo_name: str):
    """
    공통 단일 모델 트레이너.
    - DELTA_TARGET(T2)이 켜져 있으면 Δ‑target(타깃 - CUR_WARP)을 학습하고, 마지막에 CUR_WARP를 다시 더해 원 스케일로 복구한다.
    - X2E_YJ가 켜져 있으면 X2E 방향에 한해 Yeo‑Johnson으로 타깃을 변환한 뒤 역변환한다.
    - BAGGING 플래그가 켜져 있으면 _fit_predict_model_ensemble을 통해 bootstrap bagging을 수행한다.
    """
    tr = train_df.copy()
    te = test_df.copy()
    ...
    # 원 스케일 타깃 (평가 용도)
    y_tr_orig = tr[TARGET_COL].values
    y_te_orig = te[TARGET_COL].values

    # Δ‑target 여부
    if "DELTA_TARGET" in flags:
        tr["_target"] = tr[TARGET_COL] - tr[CUR_WARP]
        te["_target"] = te[TARGET_COL] - te[CUR_WARP]
        base_tr = tr[CUR_WARP].values
        base_te = te[CUR_WARP].values
    else:
        tr["_target"] = tr[TARGET_COL]
        te["_target"] = te[TARGET_COL]
        base_tr = np.zeros_like(y_tr_orig)
        base_te = np.zeros_like(y_te_orig)

    # feature selection 시 항상 CUR_WARP / pair_delta_te는 포함
    ...
    Xte_df = (
        te.drop(columns=[c for c in EXCLUDE if c in te.columns], errors="ignore")
          .select_dtypes(include=[np.number])
          .reindex(columns=cols, fill_value=np.nan)
          .fillna(med)
    )

    ytr_model = tr["_target"].values
    groups = tr[PLATE_COL].astype(str).values

    # X2E_YJ: X2E 방향에서만 Yeo-Johnson 타깃 변환
    if (direction_key == "X2E") and ("X2E_YJ" in flags):
        pt = fit_yj_transformer(ytr_model)
        ytr_t = yj_transform(pt, ytr_model)
        chosen_names, Xtr, Xte = select_features(
            Xtr_df, ytr_t, Xte_df, mandatory=mandatory, k=SELECTK_K
        )
        models, yhat_tr_t, yhat_te_t = _fit_predict_model_ensemble(
            Xtr, ytr_t, Xte, flags, algo_name, chosen_names,
            sample_weight=sample_weight, groups=groups,
        )
        pred_tr_raw = yj_inverse(pt, yhat_tr_t)
        pred_te_raw = yj_inverse(pt, yhat_te_t)
    else:
        chosen_names, Xtr, Xte = select_features(
            Xtr_df, ytr_model, Xte_df, mandatory=mandatory, k=SELECTK_K
        )
        models, pred_tr_raw, pred_te_raw = _fit_predict_model_ensemble(
            Xtr, ytr_model, Xte, flags, algo_name, chosen_names,
            sample_weight=sample_weight, groups=groups,
        )

    # Δ‑target을 사용한 경우 원 스케일로 복원
    if "DELTA_TARGET" in flags:
        yhat_tr_orig = base_tr + pred_tr_raw
        yhat_te_orig = base_te + pred_te_raw
    else:
        yhat_tr_orig = pred_tr_raw
        yhat_te_orig = pred_te_raw

    return {
        "model": models,
        "features": chosen_names,
        "r2_tr": float(r2_score(y_tr_orig, yhat_tr_orig)),
        "r2_te": float(r2_score(y_te_orig, yhat_te_orig)),
        "rmse_tr": float(np.sqrt(mean_squared_error(y_tr_orig, yhat_tr_orig))),
        "rmse_te": float(np.sqrt(mean_squared_error(y_te_orig, yhat_te_orig))),
        "yte": y_te_orig,
        "yhat": yhat_te_orig,
    }
...
def _corr_topk(df: pd.DataFrame, k=12):
    ...
    X = _sanitize_feature_columns(X).select_dtypes(include=[np.number]).copy()
    ...
    for c in X.columns:
        ...
        if np.isfinite(r): out.append((c, r, abs(r)))
    out.sort(key=lambda t: t[2], reverse=True)
    return out[:k]

def _append_corr_section(lines: list[str], label: str, df_full: pd.DataFrame, df_train_raw: pd.DataFrame, k=12):
    top_full = _corr_topk(df_full, k=k); top_tr = _corr_topk(df_train_raw, k=k)
    lines.append(f"\n[DEBUG:{label}] 분할 전 타깃-특징 상관 Top-{k} (abs)")
    for c, r, a in top_full: lines.append(f"- {c}: r={r:.4f}")
    lines.append(f"\n[DEBUG:{label}] (train-raw) 상관 Top-{k}")
    for c, r, a in top_tr:   lines.append(f"- {c}: r={r:.4f}")
    dmap_full = {c:r for c,r,_ in top_full}; dmap_tr = {c:r for c,r,_ in top_tr}
    common = set(dmap_full.keys()) & set(dmap_tr.keys())
    diffs = [(c, dmap_full[c], dmap_tr[c], dmap_tr[c]-dmap_full[c]) for c in common]
    diffs.sort(key=lambda t: abs(t[3]), reverse=True)
    if diffs:
        lines.append(f"\n[DEBUG:{label}] 전→후 상관 변화량 Top-{min(10, len(diffs))}")
        for c, rf, rt, d in diffs[:10]:
            lines.append(f"- {c}: before={rf:.4f} → train={rt:.4f} (Δ={d:+.4f})")
...
def _apply_fe_by_flags_safe(train_df: pd.DataFrame, test_df: pd.DataFrame, flags: set):
    ...
    if STRICT_NO_TRANSD:
        return tr, te
    if "SEQ_LAG" in flags:   tr = add_sequence_lags(tr); te = add_sequence_lags(te)
...
def _run_one(key: str, flags: set, tag: str, e2x_raw_all: pd.DataFrame, x2e_raw_all: pd.DataFrame, anchor_base: str):
    # 1) Split (FE 이전)
    if FIXED_ANCHOR_NAME:
        anchor_name = FIXED_ANCHOR_NAME
    else:
    ...
    x2e_res = (train_x2e_boosted(x2e_tr, x2e_te, flags) if use_boost
               else train_single_model(x2e_tr, x2e_te, "X2E", flags, ALGO_FIXED["X2E"]))
    ...
    if GLOBAL_DIAG["dropped_cols_by_guard"]:
        lines.append("\n[LEAK-GUARD] 이름 패턴으로 제거된 열")
        for c in sorted(set(GLOBAL_DIAG["dropped_cols_by_guard"])):
            lines.append(f"- {c}")

    lines.append("\n[DEBUG] 분할 통계")
    lines.append(f"- E2X: train={len(e2x_tr_raw):5d} (plates={e2x_tr_raw[PLATE_COL].nunique()}), "
                 f"test={len(e2x_te_raw):5d} (plates={e2x_te_raw[PLATE_COL].nunique()})")
    lines.append(f"- X2E: train={len(x2e_tr_raw):5d} (plates={x2e_tr_raw[PLATE_COL].nunique()}), "
                 f"test={len(x2e_te_raw):5d} (plates={x2e_te_raw[PLATE_COL].nunique()})")
...
def _parse_recipes(run_only, run_with):
    """RUN_ONLY가 비어있지 않으면 RUN_WITH는 무시. 비어있으면 RUN_WITH만 사용."""
    recipes = {}
...
def main():
    ...
    for key, flag_list in recipes.items():
        flags = set(flag_list)
        if "X2E_MOE" in flags:   _os.environ["X2E_EXPERTS"] = "1"   # 환경 플래그도 켬
        if "X2E_RES" in flags:   _os.environ["X2E_RESIDUAL"] = "1"
        if "M2" in flags:        _os.environ["X2E_MONO"] = "1"
        print("\n" + "="*80)
        anchor_str = (FIXED_ANCHOR_NAME if FIXED_ANCHOR_NAME
                      else (f"{RUN_NAME}__{key}__anchor" if ANCHOR_PER_STEP else f"{RUN_NAME}__anchor"))
        print(f"[RUN] {key} | step={key} | flags={sorted(list(flags))} | anchor={anchor_str}")
        print("="*80)
