# -*- coding: utf-8 -*-
"""
test_no_op.py  (leak-safe, optuna-free, warning-safe, auto leak-audit, debug-patched)
...
- 디버그 패치:
  * 콘솔: 앵커 강제/퍼뮤테이션 적용 여부 출력 제거
  * 요약 .txt: 실험 설정/분할 통계/분할 전후 타깃–특징 상관(Top-k) 기록
"""
...
RUN_WITH = [
    ...
    "S0+IW", "S0+BAG",
    # X2E 강화 전략 (필요 시 조합)
    "S0+X2E_MOE", "S0+X2E_YJ", "S0+X2E_RES"
]

# ★ 두 방향 모두 XGBoost 사용 (020 결과 재현 기본값)
...
# 콘솔 미출력: PERMUTE_TARGET 적용 여부는 .txt에만 기록
PERMUTE_TARGET    = _env_on("PERMUTE_TARGET","0")
# 콘솔 미출력: 003 앵커 강제 로드 여부는 .txt에만 기록
ANCHOR_FROM_CROSS = _env_on("ANCHOR_FROM_CROSS","0")
USE_RM_TRAINONLY  = _env_on("USE_RM_TRAINONLY","0")  # 기본 OFF
# 콘솔 미출력: LEAK_AUTOCHECK 여부도 .txt에만 기록
LEAK_AUTOCHECK    = _env_on("LEAK_AUTOCHECK","1")    # 기본 ON

# (신규) X2E 강화 전략 환경 플래그
X2E_EXPERTS       = _env_on("X2E_EXPERTS","0")   # 전략 A: 페어 전문가
X2E_RESIDUAL      = _env_on("X2E_RESIDUAL","0")  # 전략 C: 2-stage residual
X2E_MONO          = _env_on("X2E_MONO","0")      # X2E에만 단조 제약(+1 on CUR_WARP)
STRICT_NO_TRANSD  = _env_on("STRICT_NO_TRANSD","0")  # 테스트 내부 참조 금지 모드

# (신규) RUN_NAME/RUN_ONLY/RUN_WITH 환경 오버라이드
...
ALIAS2FLAGS = {
    ...
    "W2":   ["HETERO_W"],
    "T2":   ["DELTA_TARGET"],
    "M2":   ["MONO_CUR"],  # 전역 단조(양 방향)
    "SEG":  ["MOE_PHASE"],
    "IW":   ["IMP_WEIGHT"],
    "BAG":  ["BAGGING"],
    # 신규: X2E 전용 전략(내부 플래그를 그대로 사용)
    "X2E_MOE": ["X2E_MOE"],   # A
    "X2E_YJ":  ["X2E_YJ"],    # B
    "X2E_RES": ["X2E_RES"],   # C
}
...
ALIAS_DESC = {
    ...
    "BAG":  "Bagging(K=5)",
    "X2E_MOE": "X2E: Pair-wise Experts with global fallback",
...
GLOBAL_DIAG = {
    ...
    "nonconsec_dropped": 0,  # load_and_merge_tplus1에서 갱신
}
...
def load_and_merge_tplus1():
    ...
    - [연속성 처리] next_pass - pass != 1 인 행은 타깃을 제거(drop)하며, 개수를 로그/요약에 기록.
...
# ----------- Yeo-Johnson 변환(Train-only) 유틸 -----------
def fit_yj_transformer(y_tr: np.ndarray):
    pt = PowerTransformer(method="yeo-johnson", standardize=False)
    pt.fit(y_tr.reshape(-1,1))  # train-only
    return pt
...
# ----------- X2E 강화 트레이너 (전략 A/B/C) -----------
def train_x2e_boosted(train_df: pd.DataFrame, test_df: pd.DataFrame, flags: set):
    """
    X2E 전용 강화 경로:
      - X2E_MOE: 페어 전문가(표본 적으면 글로벌 fallback) — train만 학습, test는 예측만
      - X2E_RES: 2-stage residual (GroupKFold OOF 안전)
      - X2E_YJ : 타깃 YJ 변환(train-only) — MOE/RES와 조합 가능
      - X2E_MONO: CUR_WARP 단조 제약(+1) — base_flags로 주입
    """
    ...
    if "X2E_MOE" in flags:
        ...
        # 역변환
        if use_yj:
            yhat_te = yj_inverse(pt, yhat_te_t)
            yhat_tr = yj_inverse(pt, yhat_tr_t)
        else:
    ...
    if "X2E_RES" in flags:
        ...
        yhat_tr_t = (oof1_t + oof2_t)  # OOF 근사
...
def train_single_model(train_df, test_df, direction_key: str, flags: set, algo_name: str):
    ...
    - (X2E 전용) 플래그로 X2E_YJ가 들어오면 이 경로에서도 YJ 적용
    """
    tr = train_df.copy(); te = test_df.copy()
...
def _corr_topk(df: pd.DataFrame, k=12):
    """df(수치형)에서 TARGET_COL과의 피어슨 상관 Top-k (abs 기준)."""
    if df.empty: return []
...
def _append_corr_section(lines: list[str], label: str, df_full: pd.DataFrame, df_train_raw: pd.DataFrame, k=12):
    """요약 .txt에 분할 전/후(raw 기준) 상관 Top-k 및 변화량 Top-k를 추가."""
    top_full = _corr_topk(df_full, k=k)
    top_tr   = _corr_topk(df_train_raw, k=k)
    lines.append(f"\n[DEBUG:{label}] 분할 전 타깃-특징 상관 Top-{k} (abs 기준)")
    for c, r, a in top_full:
        lines.append(f"- {c}: r={r:.4f}")
    lines.append(f"\n[DEBUG:{label}] (train-raw) 상관 Top-{k}")
    for c, r, a in top_tr:
        lines.append(f"- {c}: r={r:.4f}")
    # 변화량(교집합)
    dmap_full = {c:r for c,r,_ in top_full}
    dmap_tr   = {c:r for c,r,_ in top_tr}
...
def _apply_fe_by_flags_safe(train_df: pd.DataFrame, test_df: pd.DataFrame, flags: set):
    ...
    (엄격 모드) STRICT_NO_TRANSD=1이면 테스트 내부 시계열 참조(L1/PN1/DF1)는 전면 off.
    """
...
def _run_one(key: str, flags: set, tag: str, e2x_raw_all: pd.DataFrame, x2e_raw_all: pd.DataFrame, anchor_base: str):
    ...
    # 2) RM train-only plate 통계 — 기본 OFF (환경변수로 ON)
    if USE_RM_TRAINONLY:
...
def main():
    ...
    # 콘솔에는 PERMUTE_TARGET/ANCHOR_FROM_CROSS/LEAK_AUTOCHECK 등 출력하지 않음
    print("="*80)
