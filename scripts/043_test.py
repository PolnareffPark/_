...
RUN_NAME = _os.environ.get("RUN_NAME", "r2_uplift_final_v4")
...
RUN_WITH = [
    ...
    "S0+TE2+X2E_YJ+M2",    # TE2 + X2E YJ + 단조
]
...
def _fit_predict_model(Xtr, ytr, Xte, flags, algo_name, chosen_names,
    ...
    def _safe_fit(m, X, y, Xv=None, yv=None, sw=None):
        EARLY_STOP = str(os.environ.get("EARLY_STOP", "0")).lower() in {"1","true","yes","y"}
        if (algo_name == "XGBoost") and EARLY_STOP and (Xv is not None) and (yv is not None):
            try:
                # 콜백 기반(조용함). 구버전이면 ImportError/TypeError로 폴백됨.
                import xgboost as xgb
                cb = [xgb.callback.EarlyStopping(rounds=60, save_best=True, maximize=False)]
                m.fit(X, y, sample_weight=sw, eval_set=[(Xv, yv)], callbacks=cb, verbose=False)
                return m
            except Exception:
                pass
        # 일반 조용한 학습(로그 없음)
    ...
    if "BAGGING" in flags:
        ...
        for s in [SEED+i for i in range(BAG_K)]:
            m = make_one(s)
            m = _safe_fit(m, Xfit, yfit, Xval, yval, sw_fit)
...
def _env_list(varname: str, default_list: list[str]) -> list[str]:
    """환경변수가 존재하면, 비어 있어도 그것을 '명시적 지정'으로 본다."""
    if varname in os.environ:
        s = str(os.environ.get(varname, ""))
        if not s.strip():
            return []  # 명시적으로 빈 리스트
