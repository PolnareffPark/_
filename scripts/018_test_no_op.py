...
# 누수 검증: PERMUTE_TARGET=1이면 타깃 셔플하여 누수 검사
PERMUTE_TARGET = ( __import__("os").environ.get("PERMUTE_TARGET","0") == "1" )
# 003 앵커 강제 로드: ANCHOR_FROM_CROSS=1이면 003의 test plate 사용
ANCHOR_FROM_CROSS = ( __import__("os").environ.get("ANCHOR_FROM_CROSS","0") == "1" )
...
def compute_rm_stats_for_plates(plates: set) -> pd.DataFrame:
    """
    주어진 train plate 집합에 대해서만 RM(roughing) 원시데이터를 집계해
    plate-level 통계를 만든다. (mean/max/min/std/median)
    """
    D = _resolve_data_dir()
    ...
    if rm.empty:
        # train plate가 없을 수는 없지만, 방어적으로 빈 프레임 반환
        return pd.DataFrame({"RM_날판번호": []})

    # 통계 대상 숫자 컬럼(plate, pass, warping_index 류는 제외)
    ...
    rm_stats = rm.groupby("RM_날판번호")[stat_cols].agg(
...
def attach_train_only_rm_stats(train_df: pd.DataFrame, test_df: pd.DataFrame):
    """
    - train plate 집합으로만 RM 통계를 계산
    - 같은 통계를 train/test에 매핑
    - test plate가 train에 전혀 없으면 'train 전역 통계(열별 median)'로 보완
    반환: (train_df_with_rm, test_df_with_rm)
    """
    tr = train_df.copy(); te = test_df.copy()
    plates_tr = set(tr[PLATE_COL].astype(str).unique().tolist())

    rm_stats_tr = compute_rm_stats_for_plates(plates_tr)   # train-only 통계
    # 병합
    ...
    # test 보완(미존재 plate): train-only 전역 통계(median)로 메움
    if not rm_stats_tr.empty:
        ...
        for c in global_med.index:
            if c in te.columns:
                te[c] = te[c].fillna(float(global_med[c]))
            if c in tr.columns:
                tr[c] = tr[c].fillna(float(global_med[c]))  # 혹시라도 결측이 남았다면 방어
    return tr, te

# -------------------- 데이터 병합 & 타깃 t→t+1 --------------------
def load_and_merge_tplus1():
    """
    Entry/Exit GT를 (plate, pass) 단일 행으로 축약한 뒤 t→t+1 타깃을 생성한다.
    - 4-1: (plate, pass)당 1행만 남긴 뒤 shift(-1)
    - 4-2: (i) (plate, pass) 중복 금지 어서션, (ii) shift가 같은 PASS를 가리키지 않는지 검사
    - 4-3과의 정합을 위해: 여기선 RM plate-통계를 '계산하지 않는다' (train-only에서 계산/매핑)
    """
    D = _resolve_data_dir()

    # --- Ground Truth (Entry/Exit) 로딩 & 키 추출
    gt_entry = pd.read_csv(D / "entry_direction_results.csv")
    gt_exit  = pd.read_csv(D / "exit_direction_results.csv")
    for df_, lab in [(gt_entry, "entry"), (gt_exit, "exit")]:
        df_["extracted_plate"] = df_["filename"].str.extract(r"(PB\d+)")
        df_["extracted_pass"]  = df_["filename"].str.extract(r"_\d+_(\d+)").astype(int)
        df_["direction"] = lab

    # --- (plate, pass)로 축약(Entry/Exit 평균) → 4-1
    gt = pd.concat([gt_entry, gt_exit], ignore_index=True)
    gt_agg = (gt.groupby(["extracted_plate", "extracted_pass"], as_index=False)
                .agg(warping_index=("warping_index", "mean")))

    # --- FM 조인 (plate, pass 단일화가 보장된 상태)
    fm = pd.read_csv(D / "posco2_105190.csv")
    merged_fm = (pd.merge(gt_agg, fm,
    ...
    # --- 타깃 t→t+1 생성 (groupby plate, shift -1)
    ...
    # --- 4-2: 사전 진단용 어서션(중복/정렬)
    # (i) (plate, pass) 유일성
    dup_any = merged_fm.duplicated([PLATE_COL, PASS_COL]).any()
    assert not dup_any, "중복 존재: (PLATE, PASS)가 유일해야 합니다. Entry/Exit 축약을 확인하세요."

    # (ii) shift가 같은 PASS를 가리키는지 검사(타깃이 있는 행에 한해)
    # 각 plate별로 다음 pass를 계산
    merged_fm["_next_pass"] = merged_fm.groupby(PLATE_COL)[PASS_COL].shift(-1)
    # 같은 pass인지 확인
    same_pass_mask = (merged_fm["_next_pass"] == merged_fm[PASS_COL]) & merged_fm[TARGET_COL].notna()
    bad_cnt = int(same_pass_mask.sum())
    assert bad_cnt == 0, f"타깃이 같은 PASS를 가리키는 표본 {bad_cnt}개 발견 — 정렬/축약 로직을 점검하세요."
    # 임시 컬럼 제거
    merged_fm = merged_fm.drop(columns=["_next_pass"])

    # --- NaN 타깃 제거(plate 내 마지막 PASS)
    merged_fm = merged_fm.dropna(subset=[TARGET_COL]).reset_index(drop=True)

    # --- 불필요 열 정리 & 저장
    final_df = merged_fm.drop(columns=["extracted_plate", "extracted_pass", "filename", "direction"],
                              errors="ignore")
...
def add_plate_expanding_norm_train(df: pd.DataFrame, topk=20):
    """Train: 분산 기준 topk 선택 + expanding norm 계산"""
    d = df.copy().sort_values([PLATE_COL, PASS_COL])
    fm_cols = [c for c in d.columns if c.startswith("FM_") and c not in {PLATE_COL, PASS_COL, MONTH_COL}]
    if not fm_cols: return d, []
    
    # ★ Train 데이터의 분산으로 topk 선택
...
def add_plate_expanding_norm_test(df: pd.DataFrame, topk_cols: list):
    """Test: Train에서 선택한 topk_cols만 사용"""
    d = df.copy().sort_values([PLATE_COL, PASS_COL])
...
def split_by_anchor(df: pd.DataFrame, direction_key: str, use_strat: bool, anchor_name: str):
    ...
    if ANCHOR_FROM_CROSS:
        if direction_key == "E2X":
            cross_anchor = MODELS_DIR / "e2x_test_plates_tplus1.pkl"  # 003의 앵커
        else:  # X2E
            cross_anchor = MODELS_DIR / "x2e_test_plates_tplus1.pkl"  # 003의 앵커
        if cross_anchor.exists():
            test_plates = set(joblib.load(cross_anchor))
            print(f"[ANCHOR_FROM_CROSS] Loaded 003 anchor for {direction_key}: {len(test_plates)} plates")
            ...
            print(f"[ANCHOR_FROM_CROSS] Warning: {cross_anchor} not found, using default split")
...
def _apply_fe_by_flags(df: pd.DataFrame, flags: set) -> pd.DataFrame:
    """기존 함수 (하위 호환용, 사용하지 않음)"""
    d = base_feature_engineering(df)
    if "SEQ_LAG"     in flags: d = add_sequence_lags(d)
    if "PLATE_NORM"  in flags: d = add_plate_expanding_norm_train(d)[0]  # train 버전 사용
    if "DELTA_FEATS" in flags: d = add_delta_causal_features_train(d)[0]  # train 버전 사용
    return d

def _apply_fe_by_flags_safe(train_df: pd.DataFrame, test_df: pd.DataFrame, flags: set):
    """FE를 train 기준으로 적용, test는 같은 변환 매핑"""
    tr = base_feature_engineering(train_df)
    te = base_feature_engineering(test_df)
    ...
    if "PLATE_NORM" in flags:
        # train 기준 topk 선택
        tr, topk_cols = add_plate_expanding_norm_train(tr)
        te = add_plate_expanding_norm_test(te, topk_cols)
    
    if "DELTA_FEATS" in flags:
        # train 기준 topk 선택
        tr, topk_cols = add_delta_causal_features_train(tr)
        te = add_delta_causal_features_test(te, topk_cols)
...
def main():
    ...
    if PERMUTE_TARGET:
        print("[WARNING] PERMUTE_TARGET=1 → 타깃 무작위화로 누수 검사 중")
    if ANCHOR_FROM_CROSS:
        print("[INFO] ANCHOR_FROM_CROSS=1 → 003의 test plate 사용")
    print("="*80)
    ...
    if not lb.empty:
        ...
        # ★ PERMUTE 경고
        if PERMUTE_TARGET:
            max_r2 = lb["avg_test_r2"].max()
            if max_r2 > 0.05:
                print(f"\n[LEAK WARNING] PERMUTE_TARGET=1인데 최대 R²={max_r2:.4f} > 0.05 → 누수 의심!")
            else:
                print(f"\n[PERMUTE OK] 최대 R²={max_r2:.4f} ≤ 0.05 → 누수 없음")
