...
def _add_pair_te_oof(tr: pd.DataFrame, te: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Pair(p→p+1) Δ-타깃 OOF 인코딩(EB 수축).
    - 인덱스/포지션 혼용 금지: OOF는 넘파이로 포지션 기반 할당
    - 누수 차단: 각 fold의 검증행은 'fold-train'에서만 계산한 통계로 채움
    - test 셋은 train 전체 통계로만 매핑(누수 없음)
    - 그룹 수 부족 시(plates < 2) 안전 폴백: 전체 평균에 수축한 train-전체 통계 사용
    """
    tr = tr.copy()
    te = te.copy()

    # 핵심 키와 Δ-타깃
    pair_tr = _pair_key(tr)  # Series (라벨 인덱스)
    ...
    # 넘파이 → 열로 할당(포지션 정렬로 안전)
    tr["pair_te_delta_mean"] = oof

    # test 셋은 'train 전체' 통계로만 매핑(누수 없음)
    g_all = (
        pd.DataFrame({
            "pair": pair_tr.to_numpy(),
            "delta": delta_tr,
        })
        .groupby("pair", sort=False)["delta"]
        .mean()
