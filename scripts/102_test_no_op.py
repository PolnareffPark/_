...
# ★ 기본을 '강제 없음'으로 변경 (빈 문자열이면 None로 처리)
E2X_ALGO_FIXED = os.getenv("E2X_ALGO", "").strip() or None
...
def _ensure_numeric_X(df: pd.DataFrame,
    ...
    d = df.copy()
    # 숫자형 후보 수집 → 제외 제거
    num_all = d.select_dtypes(include=[np.number]).columns.tolist()
    num_all = [c for c in num_all if c not in exclude]
...
def compare_algorithms(df: pd.DataFrame, dataset_name: str):
    ...
    groups = df[PLATE_COL].astype(str).values  # ★ groups 명시
...
def _oof_pair_te(tr_df: pd.DataFrame, y_tr: np.ndarray, use_delta: bool, n_splits=5):
    """plate‑Group OOF로 pair Δ‑TE 생성 (train‑only). test에는 train 평균만 주입."""
    gkf = GroupKFold(n_splits=n_splits)
    ...
    for tr_idx, va_idx in gkf.split(tr_df, target, groups):
        ...
        te[va_idx] = [means.get(p, target[tr_idx].mean()) for p in pair[va_idx]]
...
def _pair_weights(tr_df: pd.DataFrame, y_tr: np.ndarray, use_delta: bool):
    pair = _make_pair_id(tr_df).values
    cur  = tr_df[CUR_WARP].values
    target = y_tr if not use_delta else (y_tr - cur)

    s = (
        pd.DataFrame({"pair": pair, "y": target})
        .groupby("pair")["y"].std()
        .replace(0, np.nan)
    )
    s = s.fillna(float(np.nanstd(target)))
...
def train_model(df: pd.DataFrame,
    ...
    if PASS_COL in tr.columns:
        pmax = float(tr[PASS_COL].max() or 1.0)
        tr["FM_PASS_progress"] = tr[PASS_COL] / pmax
        te["FM_PASS_progress"] = te[PASS_COL] / pmax

    # --- RM train‑only 옵션: RM_*를 완전 제거 ---
    if USE_RM_TRAINONLY:
        drop_rm = [c for c in tr.columns if c.startswith("RM_")]
        tr.drop(columns=drop_rm, inplace=True, errors="ignore")
        te.drop(columns=drop_rm, inplace=True, errors="ignore")

    # --- Δ‑TE (OOF), WLS 생성 (train‑only) ---
    y_tr_raw = tr[TARGET_COL].values
    y_te_raw = te[TARGET_COL].values

    if USE_PAIR_TE:
        te_tr, te_map, te_def = _oof_pair_te(tr, y_tr_raw, use_delta=bool(USE_DELTA_TARGET))
        tr["PAIR_TE"] = te_tr
        pair_te_test = _make_pair_id(te).map(lambda p: te_map.get(p, te_def)).astype(float).values
        te["PAIR_TE"] = pair_te_test

    w_tr = None
    if USE_HETERO_W:
        w_tr = _pair_weights(tr, y_tr_raw, use_delta=bool(USE_DELTA_TARGET))

    # --- Δ‑타깃 (train‑only 변환, 추론 시 복원) ---
    y_tr = y_tr_raw.copy()
    y_te_true = y_te_raw.copy()
    if USE_DELTA_TARGET:
        y_tr = y_tr_raw - tr[CUR_WARP].values
        y_te = y_te_raw - te[CUR_WARP].values
    else:
        y_te = y_te_raw.copy()

    # --- 이상치 가드: train 분위 경계 filter, test clip ---
    feats_tr_all = tr.drop(columns=[c for c in EXCLUDE_FROM_X if c in tr.columns], errors="ignore")
    feats_te_all = te.drop(columns=[c for c in EXCLUDE_FROM_X if c in te.columns], errors="ignore")
    # 숫자형만
    feats_tr_all = feats_tr_all.select_dtypes(include=[np.number])
    feats_te_all = feats_te_all.select_dtypes(include=[np.number])

    # train 기준 중앙값 (추후 고정 주입)
    med = feats_tr_all.median(numeric_only=True).fillna(0.0).to_dict()
    feats_tr_all = feats_tr_all.fillna(med)
    feats_te_all = feats_te_all.fillna(med)

    # 분위 경계(1%~99%) — train filter, test clip
    q_low, q_high = 0.01, 0.99
    keep_mask = np.ones(len(tr), dtype=bool)
    bounds = {}
    for c in feats_tr_all.columns:
        lo, hi = np.quantile(feats_tr_all[c].values, [q_low, q_high])
        bounds[c] = (float(lo), float(hi))
        keep_mask &= (feats_tr_all[c].values >= lo) & (feats_tr_all[c].values <= hi)

    tr = tr.loc[keep_mask].reset_index(drop=True)
    y_tr = y_tr[keep_mask]
    if w_tr is not None:
        w_tr = w_tr[keep_mask]
    feats_tr_all = feats_tr_all.loc[keep_mask].reset_index(drop=True)

    for c, (lo, hi) in bounds.items():
        if c in feats_te_all.columns:
            feats_te_all[c] = feats_te_all[c].clip(lower=lo, upper=hi)

    # --- 피처 열 고정/대치 ---
    Xtr_df, med_fixed, base_cols = _ensure_numeric_X(feats_tr_all, EXCLUDE_FROM_X)
    Xte_df, _,          _        = _ensure_numeric_X(feats_te_all, EXCLUDE_FROM_X, base_cols=base_cols, medians=med_fixed)
    ...
    Xtr = Xtr_df.iloc[:, sel_idx].to_numpy()
    Xte = Xte_df.iloc[:, sel_idx].to_numpy()
    ...
    if algorithm == "RandomForest":
        ...
        # XGB 기본 정규화 강화 (과적합 억제)
        params = dict(
            random_state=seed, tree_method="hist", n_jobs=0,
            n_estimators=600, learning_rate=0.05,
            max_depth=5, subsample=0.8, colsample_bytree=0.7,
            min_child_weight=5, reg_alpha=0.0, reg_lambda=2.0
        )
        if USE_MONO_CUR:
            mono = [1 if col == CUR_WARP else 0 for col in sel_cols]
            params["monotone_constraints"] = "(" + ",".join(str(v) for v in mono) + ")"
    ...
    model.fit(Xtr, y_tr, **fit_kwargs)

    # --- 예측/복원 ---
    y_tr_hat = model.predict(Xtr)
    y_te_hat = model.predict(Xte)
    if USE_DELTA_TARGET:
        y_tr_hat = y_tr_hat + tr[CUR_WARP].values
        y_te_hat = y_te_hat + te[CUR_WARP].values

    # --- 성능 ---
    res = {
        "train_r2":   float(r2_score(tr[TARGET_COL].values, y_tr_hat)),
        "test_r2":    float(r2_score(y_te_true, y_te_hat)),
        "train_rmse": float(np.sqrt(mean_squared_error(tr[TARGET_COL].values, y_tr_hat))),
        "test_rmse":  float(np.sqrt(mean_squared_error(y_te_true, y_te_hat))),
    ...
    joblib.dump(med_fixed,             MODELS_DIR / f"{prefix}_feature_medians_tplus1.pkl")
...
def _predict_next_from_row(row: pd.Series, bundle: dict) -> float:
    fit_cols = bundle["fit_cols"]
    X_row = pd.DataFrame([row.reindex(fit_cols)], columns=fit_cols)
    X_row = X_row.fillna({k: bundle["medians"].get(k, 0.0) for k in fit_cols}).fillna(0.0)

    sel_idx = bundle["selector"].get_support(indices=True)
