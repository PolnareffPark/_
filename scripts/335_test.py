# SVR (고정) + 누수차단형 R² 향상 전략 (322 베이스라인 확장)
...
SELECT_K      = int(os.getenv("SELECT_K", "30"))    # 322/250: 30
...
USE_Y_SCALE    = _env_on("USE_Y_SCALE", "0")       # 322/250은 0이 더 좋았음

# === 새 전략 플래그 ===
# 1) Bagging
ENSEMBLE_N       = int(os.getenv("ENSEMBLE_N", "1"))
ENSEMBLE_SUBRATE = float(os.getenv("ENSEMBLE_SUBRATE", "0.85"))  # 0<r<=1 (plate 그룹 서브샘플 비율)

# 2) Target winsorization (train-only)
USE_WINSOR_Y   = _env_on("USE_WINSOR_Y", "0")
QY_LOW         = float(os.getenv("QY_LOW", "0.01"))
QY_HIGH        = float(os.getenv("QY_HIGH", "0.99"))

# 3) Plate-balanced sample weights
USE_PLATE_BAL_W = _env_on("USE_PLATE_BAL_W", "0")

# 4) PASS별 정규화(train 통계로만)
USE_PASS_NORM   = _env_on("USE_PASS_NORM", "0")
PASS_NORM_MODE  = os.getenv("PASS_NORM_MODE", "zscore")  # "zscore" or "demean"
...
def load_and_merge_tplus1():
    ...
    required_files = [
        "entry_direction_results.csv",
        "exit_direction_results.csv",
        "posco2_105190.csv",
        "posco1_105190.csv"
    ]
    missing = [f for f in required_files if not (DATA_DIR / f).exists()]
    if missing:
        raise FileNotFoundError(f"CRITICAL: Missing required data files in {DATA_DIR.resolve()}: {missing}")
...
def _prepare_design(tr_df: pd.DataFrame, te_df: pd.DataFrame, direction: str):
    ...
    # (새) PASS별 정규화: train 통계로만
    if USE_PASS_NORM:
        # train: 같은 PASS 행의 평균/표준편차로 정규화
        g_tr = tr[PASS_COL].astype(int)
        df_tmp = Xtr.copy()
        df_tmp["__g__"] = g_tr.values
        mu_tr = df_tmp.groupby("__g__").transform("mean").drop(columns="__g__", errors="ignore")
        if PASS_NORM_MODE == "demean":
            Xtr = Xtr - mu_tr
        else:
            sd_tr = df_tmp.groupby("__g__").transform("std").drop(columns="__g__", errors="ignore").replace(0, 1.0)
            Xtr = (Xtr - mu_tr) / sd_tr

        # test: train PASS별 통계로만 변환
        mu_by_g = Xtr.groupby(g_tr).mean()
        if PASS_NORM_MODE == "demean":
            def _norm_row(row, g):
                mu = mu_by_g.loc[g] if g in mu_by_g.index else Xtr.mean()
                return (row - mu)
        else:
            sd_by_g = Xtr.groupby(g_tr).std().replace(0, 1.0)
            def _norm_row(row, g):
                mu = mu_by_g.loc[g] if g in mu_by_g.index else Xtr.mean()
                sd = sd_by_g.loc[g] if g in sd_by_g.index else Xtr.std().replace(0, 1.0)
                return (row - mu) / sd
        pass_te_map = te[PASS_COL].astype(int).to_dict()
        Xte = Xte.apply(lambda r: _norm_row(r, pass_te_map[r.name]), axis=1)
...
def _winsorize_y(y: np.ndarray, qlo: float, qhi: float):
    qlo_v, qhi_v = np.quantile(y, [qlo, qhi])
    return np.clip(y, qlo_v, qhi_v), float(qlo_v), float(qhi_v)

def _plate_balanced_weights(tr_df2: pd.DataFrame):
    cnt = tr_df2[PLATE_COL].astype(str).value_counts()
    w = tr_df2[PLATE_COL].astype(str).map(lambda p: 1.0 / max(cnt.get(p, 1), 1)).to_numpy(float)
    # 평균 1로 정규화
...
def _fit_one_direction(df: pd.DataFrame, direction: str):
    ...
    ytr_raw = tr_df2[TARGET_COL].to_numpy(dtype=float)
    groups = tr_df2[PLATE_COL].astype(str).to_numpy()

    # (옵션) target winsorization (train-only)
    if USE_WINSOR_Y:
        ytr_w, ylo, yhi = _winsorize_y(ytr_raw, QY_LOW, QY_HIGH)
    else:
        ytr_w = ytr_raw

    # (옵션) y scaling
    if USE_Y_SCALE:
        y_mu, y_sc = _y_scale_fit(ytr_w, "mad")
        ytr_s = (ytr_w - y_mu) / y_sc
    else:
        y_mu = y_sc = None
        ytr_s = ytr_w
    ...
    # plate-balanced weights
    base_sw_full = _plate_balanced_weights(tr_df2) if USE_PLATE_BAL_W else None

    # Bagging loop
    bag_tr_preds = []
    bag_te_preds = []
    for b in range(max(1, ENSEMBLE_N)):
        # 그룹 서브샘플링
        if ENSEMBLE_SUBRATE >= 1.0 or ENSEMBLE_N == 1:
            keep_g = uniq_groups
        else:
            m = max(1, int(np.ceil(len(uniq_groups) * ENSEMBLE_SUBRATE)))
            keep_g = rng.choice(uniq_groups, size=m, replace=False)
        msk = np.isin(groups, keep_g)

        # 선택 인덱스 (bag별 seed로 다양성)
        seed_b = int(SEED + 31*b)
        if SELECT_MODE == "kbest":
            idx = _select_kbest_indices(Xtr.loc[msk], ytr_s[msk], k_used)
        elif SELECT_MODE == "mi":
            idx = _select_mi_indices(Xtr.loc[msk], ytr_s[msk], k_used, seed_b)
        elif SELECT_MODE == "union":
            idx = _select_union_indices(Xtr.loc[msk], ytr_s[msk], k_used, seed_b)
        else:
            idx = _select_union_indices(Xtr.loc[msk], ytr_s[msk], k_used, seed_b)

        selector = IndexSelector(idx, pre_cols)
        Xtr_sel_full = selector.transform(Xtr.values)
        Xte_sel      = selector.transform(Xte.values)

        Xtr_sel = Xtr_sel_full[msk]
        ytr_b   = ytr_s[msk]
        sw_b    = None if base_sw_full is None else base_sw_full[msk]

        model = _build_svr_pipeline()
        _fit_model(model, Xtr_sel, ytr_b, sample_weight=sw_b)

        # 전체 train/test에 대한 예측(훈련셋은 in-sample, bag 평균용)
        p_tr = model.predict(Xtr_sel_full)
        p_te = model.predict(Xte_sel)
        bag_tr_preds.append(p_tr)
        bag_te_preds.append(p_te)

    # bag 평균
    ytr_hat_s = np.mean(np.vstack(bag_tr_preds), axis=0)
    yte_hat_s = np.mean(np.vstack(bag_te_preds), axis=0)

    # y scale 복원
    if USE_Y_SCALE:
        ytr_hat = ytr_hat_s * y_sc + y_mu
        yte_hat = yte_hat_s * y_sc + y_mu
    else:
        ytr_hat = ytr_hat_s
        yte_hat = yte_hat_s
    ...
    pd.to_pickle([pre_cols[i] for i in selector.get_support(indices=True)], MODELS_DIR / f"{prefix}_sel_cols.pkl")
...
def write_summary(experiment_id: str, results: dict, dropped_shift: int, summary_path: Path):
    ...
    lines.append(f"- 새 전략 | ENSEMBLE_N={ENSEMBLE_N} (subrate={ENSEMBLE_SUBRATE}) | USE_WINSOR_Y={1 if USE_WINSOR_Y else 0} "
                 f"(qy={QY_LOW}-{QY_HIGH}) | USE_PLATE_BAL_W={1 if USE_PLATE_BAL_W else 0} | "
                 f"USE_PASS_NORM={1 if USE_PASS_NORM else 0} (mode={PASS_NORM_MODE})")
    ...
    for tag in ("E2X","X2E"):
        r = results[tag]
        lines.append(f"- [{tag}] k_used={r.get('k_used','?')} | select_mode={r.get('select_mode','?')} | ensemble_n={r.get('ensemble_n','?')}")
...
def main():
    ...
    perm_e2x = permutation_r2_once(e2x_tr_raw, e2x_te_raw, "E2X") if LEAK_AUTOCHECK else np.nan
    perm_x2e = permutation_r2_once(x2e_tr_raw, x2e_te_raw, "X2E") if LEAK_AUTOCHECK else np.nan
