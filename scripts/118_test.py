...
def _algo_build(name: str):
    """
    SVR/Ridge/Lasso/Linear 전용 빌더.
    - 모든 모델 앞에 StandardScaler를 붙인 파이프라인을 사용(스케일 차이 완화).
    - sample_weight는 아래 _fit_model에서 지원되는 모델에만 전달(try/except)합니다.
    """
    nm = str(name).strip().lower()
    if nm == "svr":
        # SVR은 스케일 영향이 커서 표준화 필수
        ...
        # Lasso는 일부 버전에서 sample_weight 미지원 → _fit_model에서 안전하게 처리
        return make_pipeline(StandardScaler(with_mean=True, with_std=True),
                             Lasso(alpha=0.01, random_state=SEED, max_iter=10000))
    elif nm == "linear":
        return make_pipeline(StandardScaler(with_mean=True, with_std=True),
                             LinearRegression())
    else:
        raise ValueError(f"Unknown algorithm: {name} (allowed: SVR|Ridge|Lasso|Linear)")

def _fit_model(model, X, y, sample_weight=None):
    """
    sample_weight 지원 여부와 무관하게 안전 적합.
    지원 안 하면 TypeError로 떨어지므로 무시하고 fit만 진행.
    """
    try:
...
def split_e2x_x2e(final_df: pd.DataFrame):
    """허용한 전이만 남겨서 E2X/X2E를 분리 저장한다."""
    # E2X: 홀수 PASS 중 {5,7,9}만
    e2x_df = final_df[
        (final_df[PASS_COL] % 2 == 1) & (final_df[PASS_COL].isin(ALLOWED_E2X_PASSES))
    ].copy()

    # X2E: 짝수 PASS 중 {6,8}만
    x2e_df = final_df[
        (final_df[PASS_COL] % 2 == 0) & (final_df[PASS_COL].isin(ALLOWED_X2E_PASSES))
    ].copy()
...
def _fit_one_direction(df: pd.DataFrame, direction: str, algo_name: str):
    """direction: 'E2X' or 'X2E' — 허용된 전이만 사용해 학습/평가."""
    d0 = df.copy()
    ...
    # 4) (옵션) TE/WLS — 이번 제한 실험에서는 보통 사용 안함(플래그로 제어)
    tr_df = tr_df_raw.copy()
    te_df = te_df_raw.copy()
    if USE_PAIR_TE:
        tr_df, te_df = _add_pair_te_oof(tr_df, te_df)

    # 5) 디자인 행렬 + 누수 스크리너(이름 스멜/중앙값/상관컷 포함)
    tr_df2, Xtr, Xte, med, pre_cols = _prepare_design(tr_df, te_df)  # 103 포맷: pre_cols 반환 :contentReference[oaicite:3]{index=3}
    ytr = tr_df2[TARGET_COL].to_numpy(dtype=float)

    # 6) 안정적인 K-best(과적합 방지용 가벼운 선택)
    k = max(1, min(50, Xtr.shape[1]))  # 50개 캡
    ...
    # 7) 모델 구성/학습(선형/커널) — sample_weight 미지원 모델 고려하여 안전 호출
...
def strict_rollforward_r2(all_df: pd.DataFrame):
    ...
    for plate, sub in all_df.groupby(PLATE_COL):
        ...
        for p in idx.index:
            ...
            X_row = X_row_df.values  # ← selector.transform() 금지!
