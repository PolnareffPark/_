"""
084_test.py  (clean, leak-safe, flag-aware, anchor-fixed)
...
RUN_NAME   = os.environ.get("RUN_NAME", "test")
...
def _hard_leak_guard(cols: list[str]) -> list[str]:
    ...
    bad_kw = ("target","label","oof","te_","_te","pred","_hat","_target", '_y', 'y')
    out=[]
    for c in cols:
        low = c.lower()
...
def main():
    ...
    ts = datetime.now()
    summary_path = SUM_DIR / f"{experiment_id}_084_{RUN_NAME}_summary.txt"
