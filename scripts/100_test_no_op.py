...
def compare_algorithms(df: pd.DataFrame, dataset_name: str):
    ...
    use_cols = [c for c in df.columns if c not in EXCLUDE_FROM_X]
    use_cols = _hard_leak_guard(use_cols)
    X_all = df[use_cols].select_dtypes(include=[np.number]).copy()
    ...
    algos = {
        "XGBoost": XGBRegressor(
            ...
            subsample=0.8, colsample_bytree=0.8, min_child_weight=8, reg_lambda=1.5
        ),
        "RandomForest": RandomForestRegressor(
            random_state=42, n_estimators=500, n_jobs=-1,
            max_depth=18, min_samples_leaf=4, max_features="sqrt"
        ),
    }
    ...
    for name, model in algos.items():
        scores = cross_val_score(model, X_all, y, cv=gkf, groups=groups, scoring="r2", n_jobs=-1)
        print(f"  {name:12s}: R2 = {scores.mean():.4f} ± {scores.std():.4f}")
        if scores.mean() > best_score:
            best_score, best_name = float(scores.mean()), name
...
def train_model(df: pd.DataFrame,
                ...
                algorithm: str = "XGBoost",  # "XGBoost" or "RandomForest"
                seed: int = 42,
                test_size: float = 0.2,
                select_k: int = 120):
    """
    GroupShuffleSplit(plate)로 분할한 뒤에만:
      - PASS 진행도(train max)
      - 중앙값 대치(train)
      - 분위 클리핑(train 경계로 train/test 공용)
      - (옵션) PAIR Δ‑TE OOF + (옵션) Hetero WLS
      - (옵션) CUR_WARP 단조 제약(+1, XGB만)
      - SelectKBest(f_regression) → 안정 k개
    저장물: 모델/셀렉터/중앙값/fit_cols/선택컬럼/테스트 판
    """
    from sklearn.model_selection import GroupShuffleSplit
    from sklearn.feature_selection import SelectKBest, f_regression
    from sklearn.metrics import r2_score, mean_squared_error

    # ---- ENV 스위치 읽기 (문자/숫자 섞임 방지) ----
    def _on(name, default="0"):
        v = os.getenv(name, default).strip().lower()
        return v in ("1","true","yes","y","on")

    USE_DELTA = _on("USE_DELTA_TARGET", "0")
    USE_TE    = _on("USE_PAIR_TE", "0")
    USE_W     = _on("USE_HETERO_W", "0")
    USE_MONO  = _on("USE_MONO_CUR", "0")
    # RM_TRAINONLY는 여기서는 컬럼을 추가/제거하지 않음(데이터 병합 단계에서 제어하는 구조)
    # 필요하면 te/w 와 유사하게 여기서 train-only로 주입하도록 확장 가능
    ...
    anchor_path = MODELS_DIR / (f"{anchor_tag}_{prefix}_test_plates_tplus1.pkl" if anchor_tag else f"{prefix}_test_plates_tplus1.pkl")
    ...
    # ---- 베이스 수치 피처 집합(라벨/식별자 제거 후) ----
    base_tr = tr_df_raw.drop(columns=[c for c in EXCLUDE_FROM_X if c in tr_df_raw.columns], errors="ignore")
    base_te = te_df_raw.drop(columns=[c for c in EXCLUDE_FROM_X if c in te_df_raw.columns], errors="ignore")
    # 라벨 스멜 제거
    safe_cols = _hard_leak_guard(base_tr.columns.tolist())
    base_tr = base_tr[safe_cols].select_dtypes(include=[np.number]).copy()
    base_te = base_te[safe_cols].select_dtypes(include=[np.number]).copy()

    # ---- 중앙값(train), 열순서 고정 ----
    def _ensure_numeric_X_local(d: pd.DataFrame, med: dict | None, cols: list | None):
        X = d.copy()
        if cols is None:
    ...
    Xtr_df, medians, fit_cols = _ensure_numeric_X_local(base_tr, med=None, cols=None)
    Xte_df, _,        _       = _ensure_numeric_X_local(base_te, med=medians, cols=fit_cols)

    # ---- 분위 클리핑 경계(train) → train/test 공용 ----
    q_lo, q_hi = 0.01, 0.99
    bounds = {}
    for c in Xtr_df.columns:
        lo, hi = np.quantile(Xtr_df[c].values, [q_lo, q_hi])
        bounds[c] = (float(lo), float(hi))
    def _clip_inplace(X: pd.DataFrame):
        for c,(lo,hi) in bounds.items():
            X[c] = X[c].clip(lower=lo, upper=hi)
    _clip_inplace(Xtr_df)
    _clip_inplace(Xte_df)

    # ---- 타깃 구성(+Δ 옵션) ----
    y_tr_true = tr_df_raw[TARGET_COL].values
    y_te_true = te_df_raw[TARGET_COL].values
    if USE_DELTA:
        y_tr = y_tr_true - tr_df_raw[CUR_WARP].values
        y_te = y_te_true - te_df_raw[CUR_WARP].values
    else:
        y_tr = y_tr_true.copy()
        y_te = y_te_true.copy()

    # ---- (옵션) PAIR Δ-TE (OOF) & (옵션) WLS 가중 ----
    # TE는 '추가 특성'이 아니라 '잔차 분해(베이스라인+잔차 모델링)'로 넣습니다.
    te_oof = np.zeros(len(tr_df_raw), dtype=float)
    te_map, te_default = {}, 0.0
    if USE_TE:
        pair = (tr_df_raw[PASS_COL].astype(int).astype(str) + "→" + (tr_df_raw[PASS_COL]+1).astype(int).astype(str)).values
        from sklearn.model_selection import GroupKFold
        gkf = GroupKFold(n_splits=5)
        groups = tr_df_raw[PLATE_COL].astype(str).values
        for tr_i, va_i in gkf.split(Xtr_df, y_tr, groups):
            df_tmp = pd.DataFrame({"pair": pair[tr_i], "y": y_tr[tr_i]})
            means = df_tmp.groupby("pair")["y"].mean()
            te_oof[va_i] = [means.get(p, df_tmp["y"].mean()) for p in pair[va_i]]
        # 전체(train) 평균 맵(→ test에 사용)
        te_map = pd.DataFrame({"pair": pair, "y": y_tr}).groupby("pair")["y"].mean().to_dict()
        te_default = float(np.mean(y_tr))
    # 잔차 타깃
    if USE_TE:
        y_tr_res = y_tr - te_oof
    else:
        y_tr_res = y_tr

    # (옵션) 이분산 가중: pair별 잔차 분산의 역수
    sample_w = None
    if USE_W:
        pair_tr = (tr_df_raw[PASS_COL].astype(int).astype(str) + "→" + (tr_df_raw[PASS_COL]+1).astype(int).astype(str)).values
        dtmp = pd.DataFrame({"pair": pair_tr, "y": y_tr_res})
        std_map = dtmp.groupby("pair")["y"].std().replace(0, np.nan)
        global_std = float(dtmp["y"].std())
        std_map = std_map.fillna(global_std)
        var_map = (std_map ** 2 + 1e-6).to_dict()
        sample_w = np.array([1.0/var_map.get(p, (global_std**2 + 1e-6)) for p in pair_tr], dtype=float)

    # ---- 피처 선택(안정 k) ----
    selector = SelectKBest(score_func=f_regression, k=min(select_k, Xtr_df.shape[1]))
    selector.fit(Xtr_df, y_tr_res)
    sel_idx = selector.get_support(indices=True)
    sel_cols = [Xtr_df.columns[i] for i in sel_idx]
    ...
    if algorithm == "RandomForest":
        ...
        # XGB: early_stopping을 못 쓰는 환경 대비하여 강한 정규화
        mono = None
        if USE_MONO and (CUR_WARP in sel_cols):
            mono = tuple(1 if c == CUR_WARP else 0 for c in sel_cols)
        model = XGBRegressor(
            random_state=seed, tree_method="hist", n_jobs=0,
            n_estimators=400, learning_rate=0.05,
            max_depth=5, subsample=0.75, colsample_bytree=0.75,
    ...
    # ---- 학습(잔차 대상) ----
    model.fit(Xtr_sel, y_tr_res, sample_weight=sample_w)

    # ---- 예측 및 원복 ----
    tr_res_pred = model.predict(Xtr_sel)
    te_res_pred = model.predict(Xte_sel)

    if USE_TE:
        # test의 TE 베이스라인
        pair_te = (te_df_raw[PASS_COL].astype(int).astype(str) + "→" + (te_df_raw[PASS_COL]+1).astype(int).astype(str)).values
        te_base_te = np.array([te_map.get(p, te_default) for p in pair_te], dtype=float)
        y_te_pred_base = te_base_te + te_res_pred
        y_tr_pred_base = te_oof + tr_res_pred
    else:
        y_tr_pred_base = tr_res_pred
        y_te_pred_base = te_res_pred

    # Δ 사용 시 원 타깃으로 복원
    if USE_DELTA:
        y_tr_pred = y_tr_pred_base + tr_df_raw[CUR_WARP].values
        y_te_pred = y_te_pred_base + te_df_raw[CUR_WARP].values
    else:
        y_tr_pred = y_tr_pred_base
        y_te_pred = y_te_pred_base
    ...
    res = {
        ...
        "train_samples": int(len(y_tr_true)),
    ...
    joblib.dump(medians,                    MODELS_DIR / f"{prefix}_feature_medians_tplus1.pkl")
    ...
    # (옵션) TE 베이스라인 맵 저장(롤포워드/추론 일관성)
    if USE_TE:
        joblib.dump({"map": te_map, "default": float(te_default)}, MODELS_DIR / f"{prefix}_pair_te_map_tplus1.pkl")
...
def _load_bundle(prefix: str) -> dict:
    ...
    te_map_p = MODELS_DIR / f"{prefix}_pair_te_map_tplus1.pkl"
    te_info  = joblib.load(te_map_p) if te_map_p.exists() else None
    return {"model": model, "selector": selector, "medians": medians, "fit_cols": list(fit_cols), "te": te_info}

def _predict_next_from_row(row: pd.Series, bundle: dict) -> float:
    """
    학습 당시 selector '입력 전체 열 순서(fit_cols)'로 1행 DF를 만든 뒤
    selector.get_support(mask)로 직접 열 슬라이스 → ndarray → model.predict.
    (sklearn의 feature name 검사 우회)
    """
    ...
    sel_idx = selector.get_support(indices=True) if hasattr(selector, "get_support") else np.arange(len(fit_cols))
