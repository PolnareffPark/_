...
def _build_xgb(robust: bool = False, monotone_index=None, n_features=None, seed=SEED):
    """
    - eval_metric은 생성자에서 지정(버전 호환)
    - n_jobs=-1: 모든 코어 사용
    - tree_method='hist': 속도 개선
    - 조기중단은 _fit_xgb_safe()에서 처리(버전별 분기)
    """
    params = dict(
        random_state=seed,
        tree_method="hist",
        n_estimators=600,           # 조기중단이 있으면 실제 사용 트리는 더 적음
        learning_rate=0.05,
        max_depth=5,
        subsample=0.8,
        ...
        reg_lambda=1.0,
        reg_alpha=0.0,
        n_jobs=-1,                 # <-- 중요: 전체 코어 사용
        eval_metric="rmse",
        verbosity=0,               # fit(verbose=...) 대신 생성자에서 제어
    )
...
def _fit_predict_model(Xtr, ytr, Xte, flags, algo_name, chosen_names,
    ...
    """
    - 내부 홀드아웃을 plate-group 기준으로 생성하여 (가능하면) 조기중단에 사용
    - XGBoost는 _fit_xgb_safe()로 버전 무관 조기중단 처리
    - 조기중단 불가 환경에서는 n_estimators를 자동 축소(최대 300)
    """
    ...
    def make_one(seed):
        if algo_name == "XGBoost":
            return _build_xgb(("ROBUST_LOSS" in flags),
                              monotone_index=mono_idx,
                              n_features=len(chosen_names),
                              seed=seed)
        else:
            return _build_rf(seed=seed)

    # 내부 홀드아웃 (plate-group) for early-stopping
    ...
    if "BAGGING" in flags:
        ...
        for s in [SEED + i for i in range(BAG_K)]:
            m = make_one(s)
            if algo_name == "XGBoost":
                m = _fit_xgb_safe(m, Xfit, yfit, sample_weight=sw_fit, Xval=Xval, yval=yval, early_rounds=50)
            else:
                m.fit(Xfit, yfit, sample_weight=sw_fit)
            preds.append(m.predict(Xte))
            last_model = m
        return last_model, np.mean(preds, axis=0)
