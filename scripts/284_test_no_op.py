...
def _make_pair_id(df: pd.DataFrame) -> pd.Series:
    return df["FM_PASS NO N"].astype(int).astype(str) + "→" + (df["FM_PASS NO N"] + 1).astype(int).astype(str)

def _oof_pair_te_smooth(tr_df: pd.DataFrame, y_tr: np.ndarray, use_delta: bool, alpha: float = 10.0, n_splits=5):
    """plate-group OOF pair 평균 + EB 수축. 누수 방지.
       반환: te_tr(OOB), te_map(test용), global_mean(default)"""
    gkf = GroupKFold(n_splits=n_splits)
    groups = tr_df["FM_날판번호"].astype(str).values
    pair   = _make_pair_id(tr_df).values
    cur    = tr_df["warping_index_current_pass"].values
    ...
    for tr_idx, va_idx in gkf.split(tr_df, target, groups):
        ...
        agg        = pd.DataFrame({"pair": p_tr, "y": y_tr_fold}).groupby("pair")["y"].agg(["mean","count"])
    ...
    agg_all      = pd.DataFrame({"pair": pair, "y": target}).groupby("pair")["y"].agg(["mean","count"])
...
def _pair_weights(tr_df: pd.DataFrame, y_tr: np.ndarray, use_delta: bool):
    pair = _make_pair_id(tr_df).values
    cur  = tr_df["warping_index_current_pass"].values
    target = y_tr - cur if use_delta else y_tr
    s = pd.DataFrame({"pair": pair, "y": target}).groupby("pair")["y"].std().replace(0, np.nan)
    s = s.fillna(s.mean())
    w = 1.0 / (s**2 + 1e-6)
    w_map = w.to_dict()
...
def _quantile_clip_train_apply(Xtr: pd.DataFrame, Xte: pd.DataFrame, qlo: float, qhi: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    lo = Xtr.quantile(qlo); hi = Xtr.quantile(qhi)
    return Xtr.clip(lower=lo, upper=hi, axis=1), Xte.clip(lower=lo, upper=hi, axis=1)


def _corr_prune_train_apply(Xtr: pd.DataFrame, Xte: pd.DataFrame, thr: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    corr = Xtr.corr(numeric_only=True).abs()
    ...
    for i, ci in enumerate(cols):
        ...
        for j in range(i+1, len(cols)):
            ...
            if cj in to_drop: 
                continue
            if upper[i, j] and corr.iat[i, j] >= thr:
                # 간단 규칙: 분산 작은 쪽 제거
                drop = cj if Xtr[cj].var(ddof=0) <= Xtr[ci].var(ddof=0) else ci
                to_drop.add(drop)
...
def train_model(
    ...
    # 1) 사용 열 및 split
    EXCLUDE_FROM_X = {"warping_index_target", "FM_날판번호", "FM_PASS NO N", "FM_압연월"}
    ...
    y_all     = df["warping_index_target"].values
    groups    = df["FM_날판번호"].astype(str).values
    ...
    # 2) 중앙값 대치 (train-only 통계)
    med = X_tr.median(numeric_only=True).to_dict()
    X_tr = X_tr.fillna(med)
    X_te = X_te.fillna(med)

    # 3) (옵션) OOF Pair TE (누수 방지, EB 수축)
    if use_pair_te:
        te_tr, te_map, te_def = _oof_pair_te_smooth(tr_df, tr_df["warping_index_target"].values, use_delta=False, n_splits=5)
        te_pairs = _make_pair_id(te_df).values
        te_te    = np.array([te_map.get(p, te_def) for p in te_pairs], dtype=float)
        # fragmentation 회피: concat 1회로 추가
    ...
    selector.fit(Xtr_s, tr_df["warping_index_target"].values)
    ...
    sample_weight = None
    if use_hetero_w:
        sample_weight = _pair_weights(tr_df, tr_df["warping_index_target"].values, use_delta=False).astype(float)

    # 9) SVM
    C     = float(os.getenv("SVR_C", "10.0"))
    EPS   = float(os.getenv("SVR_EPS", "0.2"))
    GAMMA = os.getenv("SVR_GAMMA", "scale").strip()
    try:
    ...
    svr.fit(Xtr_sel, tr_df["warping_index_target"].values, sample_weight=sample_weight)
    ...
    res = {
        "algorithm":      "SVM",
        "train_r2":       float(r2_score(tr_df["warping_index_target"].values, y_tr_pred)),
        "test_r2":        float(r2_score(te_df["warping_index_target"].values, y_te_pred)),
        "train_rmse":     float(np.sqrt(mean_squared_error(tr_df["warping_index_target"].values, y_tr_pred))),
        "test_rmse":      float(np.sqrt(mean_squared_error(te_df["warping_index_target"].values, y_te_pred))),
