...
def compare_algorithms(df: pd.DataFrame, dataset_name: str):
    ...
    for name, model in algos.items():
        scores = cross_val_score(model, X, y, cv=gkf, scoring="r2", n_jobs=-1, groups=groups)
...
def train_model(df,
                ...
                select_k: int = 120):
    """
    - plate 기준 GroupShuffleSplit으로 분할 (라벨 누수 방지)
    - 중앙값 대치: train 중앙값으로만 계산→train/test 공용 적용(누수 방지)
    - SelectKBest(f_regression) 적용: '훈련 시 사용한 입력 컬럼 순서'를 별도 저장
    - 모델 학습 후: 모델/셀렉터/입력컬럼/중앙값/test plate 저장
    """
    ...
    groups_all = df[PLATE_COL].astype(str).values
    ...
    tr_df_raw = df.iloc[tr_idx].copy()
    # te_df_raw = df.iloc[te_idx].copy()   # (필요시 사용)

    # ---- 중앙값: train 기준으로만 계산 → train/test 공용 적용
    med = X_tr_raw.median(numeric_only=True).to_dict()
    X_tr = X_tr_raw.fillna(med)
    X_te = X_te_raw.fillna(med)

    # ---- selector: 훈련 입력 컬럼 순서를 정확히 저장
    selector_input_cols = X_tr.columns.tolist()
    k_actual = max(1, min(select_k, X_tr.shape[1]))
    selector = SelectKBest(score_func=f_regression, k=k_actual)
    selector.fit(X_tr, y_tr)

    # transform은 이름검사 피하려면 numpy로 전달(순서만 맞추면 됨)
    ...
    # ---- test plate 집합 저장 (집합 연산으로 정확히)
    train_plates_set = set(tr_df_raw[PLATE_COL].astype(str).unique())
    all_plates_set   = set(df[PLATE_COL].astype(str).unique())
    test_plates      = sorted(all_plates_set - train_plates_set)

    # ---- 저장(roll‑forward에 필요한 메타 포함)
    joblib.dump(model,                      MODELS_DIR / f"{prefix}_regressor_tplus1.pkl")
    joblib.dump(selector,                   MODELS_DIR / f"{prefix}_selector_tplus1.pkl")
    joblib.dump(selector_input_cols,        MODELS_DIR / f"{prefix}_selector_input_cols_tplus1.pkl")
