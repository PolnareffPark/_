# 134_test.py 에서 시작 (SVR 고정 + 누수차단 + 전략 플래그 ON/OFF + 자동 메트릭 저장)
...
USE_HETERO_W     = _env_on("USE_HETERO_W", "0")       # 이분산 가중치 WLS 비슷
USE_RM_TRAINONLY = _env_on("USE_RM_TRAINONLY", "0")   # test의 RM_* 직접사용 금지
USE_POLY_FEATS   = _env_on("USE_POLY_FEATS", "0")     # SVR 전 다항특성(누수 X, 파이프라인 내부)
...
PAIR_MIN_N   = int(os.getenv("PAIR_MIN_N", "1"))      # TE/Weight에 쓰일 최소 표본수
...
def _build_svr_pipeline():
    ...
    if USE_POLY_FEATS:
        # 다항 특성은 파이프라인 내부에서 train‑only 로 학습/적용(누수 X)
...
def _add_pair_te_oof(tr: pd.DataFrame, te: pd.DataFrame):
    """Pair Δ 타깃의 plate‑GroupKFold OOF 평균(EB 수축)."""
    ...
    n_groups = tr[PLATE_COL].astype(str).nunique()
    n_splits = min(5, n_groups if n_groups >= 2 else 2)
    if n_splits < 2:
        # 그룹이 너무 적으면 전체 평균만 사용
        global_mean = float(np.nanmean(delta_tr)) if len(delta_tr) else 0.0
        tr["pair_te_delta_mean"] = global_mean
        te["pair_te_delta_mean"] = global_mean
        return tr, te

    gkf = GroupKFold(n_splits=n_splits)
    for tr_i, va_i in gkf.split(tr, groups=tr[PLATE_COL].astype(str).values):
        sub = tr.iloc[tr_i]
        dfm = pd.DataFrame({"pair": _pair_key(sub), "delta": delta_tr[tr_i]})
        g_mean = dfm.groupby("pair")["delta"].mean()
        # 최소 표본 수 기준으로 EB 수축
        cnt = dfm.groupby("pair")["delta"].size()
        shrink = (cnt / np.maximum(cnt, PAIR_MIN_N)).clip(0, 1)
        g_mean = (g_mean * shrink).reindex(g_mean.index)
    ...
    # test: train 통계만 사용
    dfm_all = pd.DataFrame({"pair": pair_tr, "delta": delta_tr})
    g_mean_all = dfm_all.groupby("pair")["delta"].mean()
    cnt_all = dfm_all.groupby("pair")["delta"].size()
    shrink_all = (cnt_all / np.maximum(cnt_all, PAIR_MIN_N)).clip(0, 1)
    g_mean_all = (g_mean_all * shrink_all).reindex(g_mean_all.index)

    te_enc = pair_te.map(g_mean_all).fillna(global_mean)
    tr["pair_te_delta_mean"] = oof.values
    te["pair_te_delta_mean"] = te_enc.values
    return tr, te
...
def _fit_one_direction(df: pd.DataFrame, direction: str):
    ...
    # (선택) 번들 보관(재현/디버그용) — rollforward는 제거했으나 참고 저장
    ...
    }, (tr_df_raw, te_df_raw)  # permutation 감사용
