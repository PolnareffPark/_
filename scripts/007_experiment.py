...
def train_model(df: pd.DataFrame, dataset_name: str, algorithm: str, seed: int = 42):
    """
    train_model()과 동일하되:
      - feature_cols_tplus1.pkl 저장
      - test_plates를 함께 반환
    
    Returns:
        results (dict): train/test R2, RMSE 등 성능 지표
        test_plates (set): 테스트 세트에 속한 판번호 집합
    """
    from sklearn.model_selection import GroupShuffleSplit, cross_val_score
    from sklearn.feature_selection import SelectKBest, f_regression
    from sklearn.metrics import mean_squared_error, r2_score
    ...
    joblib.dump(feature_cols, Path("models")/f"{prefix}_feature_cols_tplus1.pkl")  # ★ 추가 저장
