...
def _load_bundle(prefix: str):
    path = Path("models")
    mdl = joblib.load(path / f"{prefix}_regressor_tplus1.pkl")
    if isinstance(mdl, tuple):  # (model, params) 형태 호환
        mdl = mdl[0]
    cols = joblib.load(path / f"{prefix}_feature_cols_tplus1.pkl")
    meds = joblib.load(path / f"{prefix}_feature_medians_tplus1.pkl")
    sel_path = path / f"{prefix}_selector_tplus1.pkl"
    selector = joblib.load(sel_path) if sel_path.exists() else None
    return {"model": mdl, "selector": selector, "cols": cols, "medians": meds}
