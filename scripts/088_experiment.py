...
def compare_algorithms(df: pd.DataFrame, dataset_name: str):
    y = df["warping_index_target"]
    X = df.drop(columns=[c for c in ["warping_index_target","FM_날판번호","FM_PASS NO N","FM_압연월"] if c in df], errors="ignore")
    X = X.select_dtypes(include=[np.number]).fillna(X.median(numeric_only=True))
    groups = df["FM_날판번호"].astype(str).values  # plate group
    ...
    # 안정화 규칙: E2X는 XGBoost 우선(단, RF가 XGB보다 0.02p 이상 높을 때만 바꿈)
    if dataset_name.upper() == "E2X":
        if results["RandomForest"][0] > results["XGBoost"][0] + 0.02:
            best = "RandomForest"
        else:
            best = "XGBoost"
    else:
    ...
    print(f"[CV] {dataset_name} | XGB={results['XGBoost'][0]:.4f} RF={results['RandomForest'][0]:.4f} → {best}")
